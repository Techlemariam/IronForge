...
import { getQuota, setQuotaFilePath, trackUsage } from '../quota-manager.js';
...
describe('quota-manager', () => {
  ...
  beforeEach(() => {
    ...
    tempFile = path.join(
      tempDir,
      `quota_usage_${Date.now()}_${Math.random().toString(36).substring(7)}.json`
    );
  ...
  describe('trackUsage', () => {
    ...
    it('should create directory if it does not exist', () => {
      ...
      const subDir = path.join(
        os.tmpdir(),
        `subdir_${Date.now()}_${Math.random().toString(36).substring(7)}`
      );
