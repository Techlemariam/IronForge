  ...
  switch (name) {
    ...
    case 'trigger_autonomous_workflow': {
        ...
        if (fs.existsSync(taskDir)) {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Quota OK. Task signal written to ${taskPath} for Antigravity.`,
              },
            ],
          };
        }
      ...
      try {
        if (!octokit) {
          throw new Error(
            'Missing GITHUB_TOKEN. Please ensure it is set in your environment variables or GitHub Secrets.'
          );
        }
