...
interface SetRowProps {
    ...
    activeSetRef: React.RefObject<HTMLDivElement> | null;
}
...
const SetRow = memo(({ 
    ...
    const getRarityBorder = (rarity: string) => {
        switch(rarity) {
            case 'poor': return 'border-rarity-poor shadow-none opacity-50';
            case 'common': return 'border-rarity-common/50 shadow-none';
            case 'uncommon': return 'border-rarity-uncommon shadow-[0_0_10px_rgba(30,255,0,0.1)]';
            case 'rare': return 'border-rarity-rare shadow-[0_0_15px_rgba(0,112,221,0.2)]';
            case 'epic': return 'border-rarity-epic shadow-[0_0_20px_rgba(163,53,238,0.3)]';
            case 'legendary': return 'border-rarity-legendary shadow-[0_0_25px_rgba(255,128,0,0.4)]';
    ...
    const getRarityText = (rarity: string) => {
      switch(rarity) {
          case 'poor': return 'text-rarity-poor';
                ...
                <div className="flex justify-between items-start">
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2 mb-1">
                            <span className={`text-xs font-serif uppercase font-bold ${isFocused ? 'text-zinc-300' : 'text-zinc-600'}`}>
...
const ActionView: React.FC<ActionViewProps> = ({ 
  ...
  const [criticalHit, setCriticalHit] = useState<{show: boolean, msg: string}>({ show: false, msg: '' });
  ...
  const maxMana = 100;
  const [manaDrain, setManaDrain] = useState<{show: boolean, amount: number}>({ show: false, amount: 0 });
  ...
  // --- VOICE ---
  const handleVoiceCommand = useCallback((type: string, value: number | string) => {
      const [exIndex, setIndex] = cursor;
  ...
  useEffect(() => {
    if (activeSetRef.current) activeSetRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, [cursor]);
  ...
  const handleToggleSet = useCallback((exIndex: number, setIndex: number) => {
    setExercises(prev => {
        ...
        if (exercise.logic === ExerciseLogic.TM_PERCENT && exercise.trainingMax && newSet.weightPct) {
        ...
        if (!newSet.completed) {
            ...
            if (setIndex < exercise.sets.length - 1) {
                ...
                if (exercise.logic === ExerciseLogic.TM_PERCENT && exercise.trainingMax && nextSet.weightPct) {
        ...
        if (newSet.completed) {
            ...
            if (exercise.id === 'ex_belt_squat' && weight >= 140) achievementContext?.unlockAchievement('deep_squat_dynasty');
            if (exercise.id === 'ex_landmine_press' && weight >= 100) achievementContext?.unlockAchievement('perfect_plates');

            if (newSet.rarity === 'legendary' || newSet.rarity === 'epic' || newSet.rarity === 'rare') {
                if (typeof newSet.reps === 'string' || (newSet.completedReps && newSet.reps && newSet.completedReps > newSet.reps)) {
                    setCriticalHit({ show: true, msg: `${newSet.rarity.toUpperCase()} Performance!` });
                    playSound('loot_epic');
                    fireConfetti();
                } else playSound('ding');
            } else playSound('ding'); 
  ...
  const stateRef = useRef({ exercises, cursor, isSystemOverheated, isBlockComplete, isManaEmpty });
  useEffect(() => {
      stateRef.current = { exercises, cursor, isSystemOverheated, isBlockComplete, isManaEmpty };
  }, [exercises, cursor, isSystemOverheated, isBlockComplete, isManaEmpty]);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        if (e.target instanceof HTMLInputElement) return;
        const { exercises: currExercises, cursor: currCursor, isSystemOverheated: currOverheated, isBlockComplete: currComplete, isManaEmpty: currManaEmpty } = stateRef.current;
        ...
        else if (e.code === 'Space' || e.code === 'Enter' || e.code === 'ArrowRight' || e.code === 'KeyD') {
            e.preventDefault();
  ...
  return (
    <div className={`flex flex-col h-full bg-[#050505] transition-colors duration-500 relative ${!isSystemOverheated && bpm ? 'bg-green-950/5' : ''}`}>
      ...
      {isResting && (
          <div className="fixed top-24 right-4 z-50 animate-slide-left">
              <div className="bg-[#111] border-2 border-zinc-700 rounded-lg p-3 shadow-[0_0_20px_rgba(0,0,0,0.8)] flex items-center gap-4">
                  ...
                  <button onClick={handleSkipRest} className="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-2 rounded text-xs font-bold uppercase">
                      Ready
                  </button>
              </div>
          </div>
          ...
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 animate-slide-up w-80">
              <div className="bg-[#111] border-2 border-cyan-500 rounded-lg p-4 shadow-[0_0_20px_rgba(6,182,212,0.3)]">
                  ...
                  <div className="flex gap-2">
                      ...
                      >
                          {apreSuggestion.type === 'INCREASE' ? <ArrowUp className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
                          {apreSuggestion.adjustment > 0 ? '+' : ''}{apreSuggestion.adjustment}kg
                      </button>
                      <button 
      ...
      {isManaEmpty && (
          <div className="fixed inset-0 z-0 pointer-events-none bg-red-900/10 animate-pulse flex items-center justify-center">
              <div className="text-red-500/20 font-black text-9xl uppercase rotate-[-15deg]">Soul Burn</div>
          </div>
      )}
      ...
      {criticalHit.show && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center pointer-events-none">
              <div className="flex flex-col items-center animate-scale-bounce">
                  <div className="text-6xl font-black italic text-[#ffd700] uppercase tracking-tighter drop-shadow-[0_0_15px_rgba(255,215,0,0.8)] stroke-black" style={{WebkitTextStroke: '2px black'}}>
                      Critical Hit!
                  </div>
      ...
      <div className="px-6 py-4 border-b border-zinc-900 bg-zinc-950/80 flex justify-between items-center sticky top-0 z-10 backdrop-blur-md">
        <div>
            <div className="flex items-center gap-2 mb-1">
                <span className={`w-2 h-2 rotate-45 border ${isSystemOverheated ? 'border-orange-500 bg-orange-900' : 'border-green-500 bg-green-900'}`}></span>
                <span className={`${isSystemOverheated ? 'text-orange-500' : 'text-green-500'} font-serif text-xs uppercase tracking-widest`}>
                    {isSystemOverheated ? 'Aggro High' : 'Stealth Mode'}
                </span>
            </div>
            <h2 className="text-xl font-serif font-bold text-zinc-100 uppercase tracking-wide text-shadow-sm">{block.name}</h2>
        </div>
        ...
        <div className="flex gap-4 items-center">
             <div className="hidden md:flex flex-col items-end">
                 ...
                 <span className={`font-mono font-bold text-lg ${manaPct < 30 ? 'text-red-500' : 'text-blue-400'}`}>
                     {mana} / {maxMana}
                 </span>
             </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8 scroll-smooth pb-32">
        <VisionRepCounter isActive={showVision} onRepCount={handleVisionRep} onClose={() => setShowVision(false)} />
        <HeartRateMonitor bpm={bpm} isConnected={isBtConnected} onConnect={onBtConnect} onSimulate={onBtSimulate} error={btError} hasAura={isElite} />
                ...
                <div key={ex.id} className="space-y-4">
                    <div className="flex items-end justify-between border-b border-zinc-900 pb-2">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-zinc-900 border border-zinc-800 rounded relative">
                                ...
                                {showPrimalRoar && <div className="absolute -top-1 -right-1 bg-red-500 rounded-full p-0.5 animate-pulse"><Flame className="w-2 h-2 text-white fill-current" /></div>}
                            </div>
                            <div>
                                <h3 className="text-lg font-serif font-bold text-zinc-200">{ex.name}</h3>
                                {showPrimalRoar && <span className="text-[9px] font-bold uppercase text-red-500 tracking-wider flex items-center gap-1"><Flame className="w-3 h-3" /> Primal Roar Active</span>}
                            </div>
                            <button onClick={() => setOpenInfoId(isInfoOpen ? null : ex.id)} className={`p-1 rounded hover:bg-zinc-800 transition-colors ${isInfoOpen ? 'text-rarity-epic' : 'text-zinc-600'}`}><Info className="w-5 h-5" /></button>
                        </div>
                        {ex.logic === ExerciseLogic.TM_PERCENT && <span className="text-xs font-mono text-zinc-600">TM: {ex.trainingMax}</span>}
                    </div>

                    {isInfoOpen && (
                        <div className="bg-[#111] border border-zinc-800 rounded p-4 text-sm animate-fade-in mb-4 shadow-xl">
                            {ex.instructions && <ul className="space-y-2 mb-4">{ex.instructions.map((inst, i) => <li key={i} className="flex gap-2 text-zinc-400"><span className="text-zinc-600 font-mono">0{i+1}.</span><span className="font-serif italic">{inst}</span></li>)}</ul>}
                        </div>
                    )}
                    ...
                    <div className="space-y-3">
                        {ex.sets.map((set, setIndex) => {
                            ...
                            const displayRarity = set.completed && set.rarity ? set.rarity : calculateRarity(set, ex.logic);
      ...
      <div className="p-6 bg-zinc-950 border-t border-zinc-900 sticky bottom-0 z-40">
          ...
          >
              {isManaEmpty 
                ...
                : isBlockComplete 
                    ? (isSystemOverheated ? 'Wait for Aggro Drop' : 'Complete Quest Step') 
