...
import { ACHIEVEMENTS, SESSIONS } from '../data';
import { AchievementCategory, TitanAttributes, ExerciseLog, MeditationLog } from '../types';
import { X, Shield, Swords, Zap, Activity, Brain, User, TrendingUp, Heart, Crown, ChevronDown, Anchor, Target, Scale, Zap as ZapIcon, Calendar, Skull, ScrollText, History } from 'lucide-react';
...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ unlockedIds, onClose, meditationLogs = [] }) => {
  ...
  const attributes = calculateTitanAttributes(unlockedIds, wellness, purchasedSkillIds, meditationLogs);
        ...
        <div className="relative w-full max-w-4xl bg-[#0f0f11] border-[3px] border-[#444] rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="relative h-12 bg-gradient-to-b from-[#2a2a2a] to-[#111] border-b border-[#555] flex items-center justify-between px-4 shrink-0 z-10">
                <div className="flex items-center gap-2">
                    ...
                    <span className={`font-bold tracking-wide ${WOW_GOLD} text-shadow-sm`}>Character Info</span>
                </div>
                <button onClick={onClose} className="text-zinc-400 hover:text-white hover:bg-red-900/50 rounded p-1 transition-colors">
                    <X className="w-5 h-5" />
                </button>
            </div>
            ...
            <div className="relative flex-1 flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="w-full md:w-[45%] bg-[#050505] relative flex flex-col border-r border-[#333]">
                    ...
                    <div className="p-4 text-center z-10 bg-gradient-to-b from-[#1a1a1a] to-transparent">
                        <h1 className="text-2xl font-bold text-white tracking-wide drop-shadow-md">Athlete Name</h1>
                        <div className="text-sm font-sans font-bold text-white flex justify-center gap-2 items-center">
                            ...
                            <span className={isElite ? 'text-[#a335ee]' : 'text-[#0070dd]'}>{currentRank.name}</span>
                            <span className={WOW_GREY}>|</span>
                    ...
                    <div className="flex-1 relative flex justify-between px-4 py-2">
                        ...
                        <div className="flex flex-col gap-2 pt-4">
                            ...
                            <GearSlot icon={<Zap className="w-5 h-5"/>} rarity={attributes.strength > 15 ? "epic" : "uncommon"} label="Shoulders" />
                            ...
                            <GearSlot icon={<Activity className="w-5 h-5"/>} rarity="uncommon" label="Tabard" />
                        </div>
                        ...
                        <div className="flex-1 flex items-center justify-center relative">
                             ...
                             <div className="relative w-40 h-64 bg-zinc-900/50 border border-zinc-700/50 rounded-lg flex flex-col items-center justify-center group overflow-hidden">
                                  ...
                                  <User className={`w-24 h-24 ${isElite ? 'text-[#FFD100]' : 'text-zinc-500'} drop-shadow-2xl`} />
                        ...
                        <div className="flex flex-col gap-2 pt-4">
                            ...
                            <GearSlot icon={<Activity className="w-5 h-5"/>} rarity={attributes.endurance > 15 ? "epic" : "rare"} label="Legs" />
                    ...
                    <div className="p-4 bg-[#111] border-t border-[#333] z-10">
                         <div className="flex justify-between text-[10px] font-bold uppercase text-zinc-400 mb-1">
                             ...
                             <span>{Math.floor(currentXP)} / {maxXP}</span>
                         </div>
                ...
                <div className="w-full md:w-[55%] bg-[#151515] p-0 overflow-y-auto custom-scrollbar relative flex flex-col">
                             ...
                             <div className="flex-1 p-6 space-y-6">
                                 ...
                                 <div>
                                     ...
                                     <div className="space-y-1">
                                         <StatRow label="Consistency" value={attributes.mental} color={attributes.mental > 10 ? WOW_GREEN : 'text-white'} subtext="Streak + Meditation" />
                                         <StatRow label="Recovery" value={attributes.recovery} color={attributes.recovery > 12 ? WOW_GREEN : 'text-white'} subtext="Sleep > 70 required" />
                                         <StatRow label="Resilience (CTL)" value={Math.max(3, Math.floor(attributes.endurance * 0.8))} color="text-zinc-400" subtext="Ramp Rate: +4" />
                                         <StatRow label="Focus" value={Math.max(5, Math.floor(attributes.mental * 1.2))} color="text-zinc-400" />
                                     </div>
                                 </div>
                                 ...
                                 <div>
                                     ...
                                     <div className="space-y-1">
                                         ...
                                         <StatRow label="Anaerobic (W')" value={Math.max(4, Math.floor(attributes.strength * 0.6))} />
                                 ...
                                 <div>
                                     ...
                                     <div className="space-y-1">
                                         ...
                                         <StatRow label="Balance" value={Math.max(5, Math.floor(attributes.technique * 0.8))} />
                                     </div>
                                 </div>
                             </div>
                         </>
                     ...
                     {activeTab === 'history' && (
                         <div className="flex-1 p-6 space-y-4">
                            ...
                            {isLoadingHistory ? (
                                <div className="text-center p-10 text-zinc-500 font-mono animate-pulse">Reading Scrolls...</div>
                                ...
                                groupedHistory.map(([date, logs]) => (
                                    <div key={date} className="bg-zinc-900/50 border border-zinc-800 rounded-lg overflow-hidden group hover:border-zinc-700 transition-colors">
                     ...
                     {activeTab === 'contract' && (
                         <div className="flex-1 p-6 flex flex-col items-center justify-center text-center">
                             ...
                             <h3 className="text-lg font-bold text-zinc-500 uppercase tracking-widest">No Active Contracts</h3>
            ...
            <div className="h-10 bg-[#111] border-t border-[#444] flex items-center justify-center gap-2 px-4 z-10">
                ...
                <TabButton label="Contract" active={activeTab === 'contract'} onClick={() => setActiveTab('contract')} />
...
const GearSlot: React.FC<{ icon: React.ReactNode, rarity: string, label: string }> = ({ icon, rarity, label }) => {
    const getBorderColor = () => {
        switch(rarity) {
            case 'epic': return 'border-[#a335ee]';
            case 'rare': return 'border-[#0070dd]';
            case 'uncommon': return 'border-[#1eff00]';
            default: return 'border-[#9d9d9d]';
        }
    };
        ...
        <div className="group relative w-10 h-10 bg-[#1a1a1a] border border-zinc-700 rounded-sm flex items-center justify-center cursor-pointer hover:brightness-125 transition-all">
            ...
            <div className="absolute left-12 top-0 hidden group-hover:block z-50 w-48 bg-[#050505] border border-zinc-600 rounded p-2 pointer-events-none shadow-xl">
                 <div className={`font-bold text-sm ${
                     rarity === 'epic' ? 'text-[#a335ee]' : rarity === 'rare' ? 'text-[#0070dd]' : rarity === 'uncommon' ? 'text-[#1eff00]' : 'text-white'
                 }`}>
                     {label} Slot
                 </div>
                 <div className="text-[10px] text-white mt-1">Item Level {Math.floor(Math.random() * 100) + 200}</div>
                 <div className="text-[10px] text-[#ffd700] mt-1">&lt;Right Click to Equip&gt;</div>
            </div>
        </div>
    );
};

const StatRow: React.FC<{ label: string, value: string | number, color?: string, tooltip?: string, subtext?: string }> = ({ label, value, color = 'text-white', tooltip, subtext }) => {
    ...
    return (
        <div className="flex justify-between items-center text-sm group relative cursor-help border-b border-[#222] py-1 last:border-0 hover:bg-[#1a1a1a] px-2 rounded">
            <div className="flex flex-col">
                <span className="text-zinc-400 group-hover:text-white transition-colors text-xs uppercase font-bold tracking-wider">{label}</span>
                {subtext && <span className="text-[9px] text-zinc-600 font-sans">{subtext}</span>}
            </div>
            <span className={`font-mono font-bold text-sm ${valueColor} bg-[#000] px-2 py-0.5 rounded border border-[#333]`}>{value}</span>
            {tooltip && (
