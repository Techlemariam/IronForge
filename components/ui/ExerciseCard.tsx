...
import { CheckCircle2, Dumbbell, Skull, Crown, Star, Sparkles } from 'lucide-react';
...
const SetItem: React.FC<SetItemProps> = ({ set, index, isActive, calculateTargetLoad }) => {
    ...
    const rarityClass = isDone ? getRarityStyles(set.rarity) : 'border-zinc-800 text-zinc-600 bg-zinc-950';

    return (
        <div className={`relative flex items-center justify-between p-3 rounded border transition-all duration-300 ${rarityClass} ${isActive ? 'bg-zinc-800/80 border-l-4 border-l-magma' : ''} ${animateClass}`}>
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                        {isDone ? (
                            <>
                                ...
                                {set.rarity === 'legendary' && <span className="text-[9px] bg-[#ff8000] text-black px-1 rounded font-black animate-pulse">PR</span>}
                            </>
                        ) : (
                            `Goal: ${set.reps} reps`
                        )}
            ...
            {isActive && !isDone && <div className="text-[9px] text-magma font-bold animate-pulse">ENGAGED</div>}
        </div>
    );
};
            ...
            <div className="bg-zinc-900/50 border-l-4 border-green-700 p-4 rounded ml-4 relative overflow-hidden group-hover:bg-zinc-900 transition-colors">
                <div className="relative z-10 flex items-center gap-6 text-xs text-zinc-400 font-mono">
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                        <span className="text-green-600 font-bold tracking-widest text-[10px] uppercase">Quest Complete</span>
                    </div>
                </div>
            </div>
        </div>
