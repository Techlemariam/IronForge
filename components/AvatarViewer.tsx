...
const HologramMaterial = ({ color, fatigue }: { color: string, fatigue: number }) => {
...
const TitanMesh: React.FC<{ attributes: TitanAttributes, isElite: boolean, heatmap: Record<string, number> }> = ({ attributes, isElite, heatmap }) => {
    ...
    return (
        <group ref={meshRef}>
            ...
            <mesh position={[0, 1.5, 0]} scale={[chestScale, 1, 0.8]}>
                ...
                <HologramMaterial color={getColor('chest')} fatigue={getFatigue('chest')} />
            </mesh>
            ...
            <mesh position={[-0.9 * chestScale, 2.1, 0]} >
                ...
                <HologramMaterial color={getColor('shoulders')} fatigue={getFatigue('shoulders')} />
            </mesh>
            <mesh position={[0.9 * chestScale, 2.1, 0]} >
                <sphereGeometry args={[0.35, 16, 16]} />
                <HologramMaterial color={getColor('shoulders')} fatigue={getFatigue('shoulders')} />
            ...
            {/* Arms */}
            <mesh position={[-0.8 * chestScale, 1.5, 0]} scale={[0.3 + (attributes.hypertrophy/50), 1.2, 0.3 + (attributes.hypertrophy/50)]}>
                <capsuleGeometry args={[0.3, 1, 4, 8]} />
                <HologramMaterial color={getColor('triceps')} fatigue={getFatigue('triceps')} />
            </mesh>
            <mesh position={[0.8 * chestScale, 1.5, 0]} scale={[0.3 + (attributes.hypertrophy/50), 1.2, 0.3 + (attributes.hypertrophy/50)]}>
                <capsuleGeometry args={[0.3, 1, 4, 8]} />
                <HologramMaterial color={getColor('triceps')} fatigue={getFatigue('triceps')} />
            ...
            <mesh position={[-0.3, 0, 0]} scale={[legScale, 1.5, legScale]}>
                ...
                <HologramMaterial color={getColor('quads')} fatigue={getFatigue('quads')} />
            </mesh>
            <mesh position={[0.3, 0, 0]} scale={[legScale, 1.5, legScale]}>
                <cylinderGeometry args={[0.25, 0.2, 1.5]} />
                <HologramMaterial color={getColor('quads')} fatigue={getFatigue('quads')} />
    ...
    return (
        <div className="w-full h-full min-h-[400px] bg-gradient-to-b from-zinc-950 to-black rounded-lg overflow-hidden relative border border-zinc-800">
            <div className="absolute top-4 left-4 z-10">
                <h3 className="text-[#c79c6e] font-bold uppercase tracking-widest text-xs">Titan Holo-Projector</h3>
            ...
            <Canvas>
                ...
                <spotLight position={[10, 10, 10]} angle={0.15} penumbra={1} intensity={2} castShadow color={isElite ? "#ffd700" : "#c79c6e"} />
                <pointLight position={[-10, 5, -10]} intensity={1} color="#00e5ff" />

                <TitanMesh attributes={attributes} isElite={isElite} heatmap={muscleHeatmap} />
                
                {/* Sci-Fi Grid Floor */}
                <gridHelper args={[20, 20, 0x444444, 0x111111]} position={[0, -0.79, 0]} />
