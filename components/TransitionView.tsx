...
import { Wrench, CheckCircle2, ArrowRight, Clock, SkipForward, Keyboard, X } from 'lucide-react';
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-4">
            {block.setupInstructions?.map((inst, idx) => (
              <li key={idx} className="flex items-start gap-4 p-4 bg-zinc-900 border border-zinc-800 rounded-lg group hover:border-orange-500/30 transition-colors">
        ...
        {setupImage ? (
           <div className="aspect-video bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden shadow-lg relative group">
              <img 
                src={setupImage} 
                alt={block.targetSetupName} 
                className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity"
              />
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-zinc-950/90 to-transparent p-4">
                  <span className="text-xs font-mono text-zinc-300 uppercase tracking-widest">Visual Reference: {block.targetSetupName}</span>
              </div>
           </div>
        ) : (
            <div className="aspect-video bg-zinc-900 border border-zinc-800 rounded-lg flex flex-col items-center justify-center text-zinc-700">
                 ...
                 <span className="font-mono text-xs uppercase">Visual Reference: {block.targetSetupName || 'N/A'}</span>
            </div>
        )}
