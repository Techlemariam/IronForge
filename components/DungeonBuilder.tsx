...
import { Session, Block, BlockType, Exercise, ExerciseLogic, Set } from '../types';
import { Plus, Trash2, Save, Dumbbell, ArrowDown, ChevronUp, ChevronDown } from 'lucide-react';
...
const DungeonBuilder: React.FC<DungeonBuilderProps> = ({ onSave, onCancel }) => {
    ...
    const updateExercise = (bIdx: number, eIdx: number, field: keyof Exercise, value: any) => {
        const updatedBlocks = [...blocks];
        updatedBlocks[bIdx].exercises![eIdx] = { ...updatedBlocks[bIdx].exercises![eIdx], [field]: value };
        setBlocks(updatedBlocks);
    };
    ...
    return (
        <div className="h-full bg-[#050505] p-6 overflow-y-auto font-serif text-zinc-200">
            <h1 className="text-3xl font-black text-[#c79c6e] mb-6 border-b border-[#46321d] pb-4">Dungeon Architect</h1>
            
            <div className="space-y-4 mb-8">
                <div className="space-y-1">
                    <label className="text-xs font-bold uppercase text-zinc-500">Dungeon Name</label>
                    <input 
                        value={name} 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold uppercase text-zinc-500">Zone / Flavor Text</label>
                    <input 
                        value={zoneName} 
            ...
            <div className="space-y-6">
                {blocks.map((block, bIdx) => (
                    <div key={block.id} className="bg-[#1a1a1a] border border-zinc-800 rounded-lg p-4 relative group transition-all">
                                    ...
                                    <div key={ex.id} className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            {ex.sets.map((set, sIdx) => (
                                                <div key={set.id} className="bg-zinc-900 px-2 py-1 rounded text-xs border border-zinc-800">
            ...
            <div className="mt-8 flex gap-4 sticky bottom-0 bg-[#050505] py-4 border-t border-zinc-900">
                <button onClick={onCancel} className="flex-1 py-3 text-zinc-500 hover:text-white">Cancel</button>
                <button 
                    onClick={handleSave}
