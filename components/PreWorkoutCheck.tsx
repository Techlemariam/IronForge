...
const PreWorkoutCheck: React.FC<PreWorkoutCheckProps> = ({ session, onProceed, onCancel }) => {
  ...
  const runScan = async (forceLow = false) => {
    ...
    try {
        ...
        if (settings && settings.intervalsApiKey && !forceLow && navigator.onLine) {
            const today = new Date().toISOString().split('T')[0];
            const wellness = await IntervalsService.getWellness(today, settings.intervalsAthleteId, settings.intervalsApiKey);
            realData = IntervalsService.mapWellnessToTitanStats(wellness);
