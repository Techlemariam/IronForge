...
import { IntervalsWellness, IntervalsActivity, WeaknessAudit, TSBForecast, ExerciseLog, TitanLoadCalculation, TTBIndices } from '../types';
...
export const AnalyticsService = {
  ...
  calculateTTB: (
    ...
    if (lastEpicSet) {
        const daysSince = (new Date().getTime() - new Date(lastEpicSet.date).getTime()) / (1000 * 3600 * 24);
    ...
    // Determine Lowest
    const scores = { strength: strengthScore, endurance: enduranceScore, wellness: wellnessScore };
  ...
  /**
   * Generates a 7-day TSB (Form) forecast based on current state and a planned heavy workout.
   * Uses a simplified Banister Impulse Response model.
   */
  forecastPRWindow: (currentWellness: IntervalsWellness, plannedSessionTSS: number = 60): TSBForecast[] => {
    const forecast: TSBForecast[] = [];
    
    // Defaults if data is missing
    let ctl = currentWellness.ctl || 45;
    let atl = currentWellness.atl || 50;
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // Apply the planned workout stress TODAY (i=0), then rest days
      const dailyStress = i === 0 ? plannedSessionTSS : 0;
   ...
   */
  calculateTitanLoad: (volumeLoad: number, avgIntensityPct: number, durationMinutes: number): TitanLoadCalculation => {
  ...
  auditWeakness: (
    ...
    if (strengthLogs.length < 3) {
        return { detected: false, type: 'NONE', message: "Insufficient data for audit.", confidence: 0 };
    }
    ...
    if (!isPlateau) {
         return { detected: false, type: 'NONE', message: "No plateau detected. Growth is steady.", confidence: 100 };
    }
    ...
    const aerobicSpike = (ctl - previousCtl) / previousCtl > 0.10;

    if (aerobicSpike) {
        return {
            detected: true,
            type: 'AEROBIC_INTERFERENCE',
            message: "Aerobic volume spike detected (+15%). This is likely dampening your neural drive for max strength.",
            confidence: 85,
    ...
    const poorRecovery = (currentWellness.sleepScore || 100) < 60 || (currentWellness.hrv || 100) < 40;
    
    if (poorRecovery) {
        return {
            detected: true,
            type: 'RECOVERY_DEBT',
            message: "Biological readiness is critically low. Your CNS cannot support PR attempts despite muscular readiness.",
            confidence: 90,
            correlationData: { metric: 'HRV / Sleep', trend: 'DOWN' }
        };
    }

    return { detected: false, type: 'NONE', message: "Plateau detected, but cause is indeterminate.", confidence: 40 };
  },
  ...
  getMockHistory: (): ExerciseLog[] => {
      return [
          { date: '2023-10-01', exerciseId: 'ex_landmine_press', e1rm: 70, rpe: 8, isEpic: true },
          { date: '2023-10-08', exerciseId: 'ex_landmine_press', e1rm: 72.5, rpe: 8.5 },
          { date: '2023-10-15', exerciseId: 'ex_landmine_press', e1rm: 72.5, rpe: 9 }, // Stagnation
          { date: '2023-10-22', exerciseId: 'ex_landmine_press', e1rm: 72.5, rpe: 9.5 }, // Stagnation + Higher RPE
      ];
  }
};
