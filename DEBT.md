...
| 2026-01-06 | `src/features/oracle/OracleChat.tsx` | Improved `context` prop from `any` to `Record<string, unknown>`. Migrated to `@ai-sdk/react` v3 API: replaced `@ts-ignore` with `DefaultChatTransport`, updated `sendMessage` to `{ text }`, and migrated `initialMessages` to `UIMessage` parts shape. | @cleanup | ✅ Resolved |
