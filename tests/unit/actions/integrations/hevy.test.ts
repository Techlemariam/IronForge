...
describe.skip("Hevy Actions (Integration)", () => {
  ...
  describe("getHevyTemplatesAction", () => {
    it("should throw error if apiKey is missing", async () => {
      await expect(getHevyTemplatesAction("")).rejects.toThrow(
    ...
    it("should return templates on success", async () => {
      ...
      vi.spyOn(libHevy, "getHevyTemplates").mockResolvedValue(
    ...
    it("should handle errors", async () => {
      vi.spyOn(libHevy, "getHevyTemplates").mockRejectedValue(
        new Error("API Error"),
      );
      await expect(getHevyTemplatesAction(apiKey)).rejects.toThrow(
        "Failed to fetch Hevy templates: API Error",
      );
    });
  });
  ...
  describe("saveWorkoutAction", () => {
    it("should save workout and award energy in DB", async () => {
      // Setup
      const payload = {
        workout: { exercises: [{ sets: [{ weight_kg: 100, reps: 10 }] }] },
      }; // 1000kg vol => 10 energy
