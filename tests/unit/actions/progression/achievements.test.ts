...
} from '@/actions/progression/achievements';
...
describe('Achievements Server Actions', () => {
  ...
  describe('checkAchievementsAction', () => {
    ...
    it('should unlock guild achievement when in guild', async () => {
      ...
      expect((result as any).newUnlocks).toBeDefined();
      expect((result as any).newUnlocks.some((a: any) => a.code === 'GUILD_MEMBER')).toBe(true);
    });
    ...
    it('should not unlock already unlocked achievements', async () => {
      (prisma.user.findUnique as any).mockResolvedValue({
        ...
        achievements: [{ achievementId: 'ach-1', achievement: { code: 'FIRST_WORKOUT' } }],
        guildId: null,
        guild: null,
      });
    ...
    it('should not unlock if conditions not met', async () => {
      ...
      expect((result as any).newUnlocks).toBeDefined();
      expect((result as any).newUnlocks.every((a: any) => a.code !== 'CARDIO_KING')).toBe(true);
    });
  });
