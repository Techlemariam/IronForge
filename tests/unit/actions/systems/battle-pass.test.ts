import * as battlePassActions from '@/actions/systems/battle-pass';
...
describe('Battle Pass Actions', () => {
    ...
    tiers: [
      {
        ...
        premiumRewardId: 'p1',
        premiumRewardData: { type: 'ITEM', itemId: 'sword' },
      },
      {
        tierLevel: 2,
        requiredXp: 200,
        ...
        freeRewardData: null,
        premiumRewardId: 'p2',
        premiumRewardData: { type: 'TITLE', titleId: 'hero' },
      },
    ],
  };
  ...
  describe('getUserBattlePassProgressAction', () => {
    ...
    it('should create new progress if missing', async () => {
      ...
      (prisma.userBattlePass.create as any).mockResolvedValue({
        ...mockUserPass,
        seasonXp: 0,
        claims: [],
      });
  ...
  describe('addBattlePassXpAction', () => {
    it('should add xp and calculate new tier', async () => {
      ...
      (prisma.userBattlePass.findUnique as any).mockResolvedValue({
        ...mockUserPass,
        seasonXp: 50,
      }); // Start with 50 XP
      ...
      // Expect Update
      expect(prisma.userBattlePass.update).toHaveBeenCalledWith(
  ...
  describe('claimBattlePassRewardAction', () => {
    it('should claim free reward successfully', async () => {
      ...
      expect(prisma.userEquipment.upsert).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { userId_equipmentId: { userId: 'user_1', equipmentId: 'sword' } },
        })
      );
      expect(prisma.userBattlePassClaim.create).toHaveBeenCalledWith(
        expect.objectContaining({
