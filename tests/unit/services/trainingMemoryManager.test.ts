...
import type { SystemMetrics, TrainingActivity } from '@/types/training';
...
describe('TrainingMemoryManager', () => {
  ...
  describe('getRewardMultiplier', () => {
    it('should return bonus for matching path', () => {
      const mult = TrainingMemoryManager.getRewardMultiplier('JUGGERNAUT', 'JUGGERNAUT');
      expect(mult).toBeGreaterThan(1.0);
    });

    it('should return 1.0 for mismatched path', () => {
      const mult = TrainingMemoryManager.getRewardMultiplier('PATHFINDER', 'JUGGERNAUT');
      expect(mult).toBe(1.0);
    });

    it('should return 1.0 for null quest path', () => {
      const mult = TrainingMemoryManager.getRewardMultiplier(null, 'JUGGERNAUT');
      expect(mult).toBe(1.0);
    });
  });
