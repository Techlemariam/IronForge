...
describe('MobilityAuditor', () => {
  ...
  const getExId = (name: string) =>
    MOBILITY_EXERCISES.find((e) => e.name.toLowerCase().includes(name.toLowerCase()))?.id || '';
  ...
  it('should detect Gold layer', () => {
    // 60 mins total
    const logs = [{ exerciseId: getExId('tibialis'), durationSecs: 3600 }];
    const report = auditMobility(logs);
    expect(report.passiveLayerLevel).toBe('GOLD');
  });

  it('should handle unknown exercise IDs gracefully', () => {
    const logs = [{ exerciseId: 'unknown-id', durationSecs: 600 }];
    const report = auditMobility(logs);
