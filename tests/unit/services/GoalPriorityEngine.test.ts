...
import type { MacroPhase, SystemMetrics, WardensManifest } from '@/types/goals';
  ...
  describe('selectPhase', () => {
    ...
    it('should respect upcoming deadline', () => {
      const deadlineManifest = {
        ...mockManifest,
        goals: [
          { goal: 'VO2MAX' as const, weight: 1.0, deadline: new Date(Date.now() + 86400000 * 10) },
        ], // 10 days
      };
...
describe('selectWorkout', () => {
  ...
  it('should filter by phase (CARDIO_BUILD -> RUN/BIKE)', () => {
    const result = GoalPriorityEngine.selectWorkout(mockManifest, 'CARDIO_BUILD', mockBudget);
  ...
  it('should filter by budget', () => {
    ...
    const result = GoalPriorityEngine.selectWorkout(mockManifest, 'CARDIO_BUILD', lowBudget);
