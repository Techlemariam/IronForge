...
describe('TitanService', () => {
  ...
  describe('modifyHealth', () => {
    ...
    it('should throw if titan not found', async () => {
      ...
      await expect(TitanService.modifyHealth('u1', -10, 'test')).rejects.toThrow('Titan not found');
    });
  });
  ...
  describe('consumeEnergy', () => {
    ...
    it('should throw if not enough energy', async () => {
      ...
      await expect(TitanService.consumeEnergy('u1', 20)).rejects.toThrow('Not enough energy');
    });

    it('should throw if titan not found', async () => {
      (prisma.titan.findUnique as any).mockResolvedValue(null);

      await expect(TitanService.consumeEnergy('u1', 10)).rejects.toThrow('Titan not found');
    });
  });
  ...
  describe('syncWellness', () => {
    ...
    it('should throw if titan not found', async () => {
      ...
      await expect(TitanService.syncWellness('u1', { bodyBattery: 50 })).rejects.toThrow(
        'Titan not found'
      );
    });
  });
  ...
  describe('updateStreak', () => {
    ...
    it('should throw if titan not found', async () => {
      ...
      await expect(TitanService.updateStreak('u1')).rejects.toThrow('Titan not found');
    });
  });
});
