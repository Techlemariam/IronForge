...
import { CONTEST_COST_GOLD, GuildTerritoryService } from '@/services/game/GuildTerritoryService';
...
describe('GuildTerritoryService', () => {
  ...
  describe('contestTerritory', () => {
    ...
    it('should throw if not enough gold', async () => {
      ...
      await expect(GuildTerritoryService.contestTerritory('g1', 't1', 'u2')).rejects.toThrow(
        /Insufficient gold/
      );
    });
  });
  ...
  describe('recordActivity', () => {
    it('should update entry if exists', async () => {
      ...
      expect(prisma.territoryContestEntry.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'e1' },
          data: {
