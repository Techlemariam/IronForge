...
import { getUserAction, updateEquipmentAction, updateGoldAction } from '@/actions/user-actions';
...
describe('Sync API Route (POST)', () => {
  ...
  it('should handle UPDATE_EQUIPMENT successfully', async () => {
    ...
    const equipment = [{ id: 'eq-1', name: 'Barbell', category: 'Barbell', isOwned: true }];
    const req = createRequest({
    ...
    expect(res.status).toBe(200);
    expect(updateEquipmentAction).toHaveBeenCalledWith(equipment);
  });
