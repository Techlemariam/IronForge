...
test.describe('Core Gameplay', () => {
  test('should allow navigating to Iron Mines (Strength)', async ({ page }) => {
    ...
    await expect(page.getByText(/Strength Log|Select an Exercise/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('should visit the Marketplace', async ({ page }) => {
    ...
    await expect(page.getByText(/Marketplace|Item Shop|Gold|Equipment/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
