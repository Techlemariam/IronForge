...
test.describe('Settings and Social Hub', () => {
  test('should navigate to Settings page and verify elements', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'System Archives' })).toBeVisible({
      timeout: 15000,
    });
  ...
  test('should navigate to Social Hub from Dashboard', async ({ page }) => {
    ...
    await expect(page.getByText(/CONNECTED|IRON NETWORK|Social|Loading/i).first()).toBeVisible({
      timeout: 15000,
    });
    ...
    const isBackAtCitadel = await trainingHeading
      .or(ironCityHeading)
      .first()
