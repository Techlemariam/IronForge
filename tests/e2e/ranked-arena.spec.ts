...
test.describe('Ranked Arena PvP', () => {
  test('should display ranked arena page with faction rank badge', async ({ page }) => {
    ...
    const rankBadge = page
      .locator('[class*="RankBadge"]')
      .or(
        page.getByText(
          /scout|private|grunt|corporal|sergeant|knight|champion|centurion|warlord|marshal|general|commander|gladiator|high warlord|grand marshal/i
        )
      )
      .first();
