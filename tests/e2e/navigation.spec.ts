...
test.describe('Navigation Smoke Tests', () => {
  test('should verify Dashboard elements', async ({ page }) => {
    ...
    await expect(page.getByText(/Iron City|Citadel|Training|Oracle/i).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('should navigate to Leaderboards', async ({ page }) => {
    ...
    await expect(page.getByText(/Iron Colosseum|Gladiators|RANK|Leaderboard/i).first()).toBeVisible(
      { timeout: 15000 }
    );
  });
