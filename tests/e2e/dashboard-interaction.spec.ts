...
test.describe('Dashboard Interactions', () => {
  ...
  test.skip('should navigate to Program Builder', async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="citadel-hub"], h3:has-text("Training"), h3:has-text("Iron City")',
      { timeout: 15000 }
    );
    ...
    const programBuilderBtn = page
      .getByRole('button', { name: /Program Builder/i })
  ...
  test('should navigate to Guild Hall', async ({ page }) => {
    ...
    const guildHallBtn = page
      .getByRole('button', { name: /Guild Hall/i })
  ...
  test('should navigate to Trophy Room', async ({ page }) => {
    ...
    await expect(page.getByText(/Trophy Room|Achievements|Hall of Fame/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
});
