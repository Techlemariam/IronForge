...
test.describe('Iron Mines - Strength Training', () => {
  ...
  test('should navigate to Iron Mines (Strength Training)', async ({ page }) => {
    ...
    const testWorkoutCard = await page.waitForSelector(
      '[data-testid="workout-card-strength_test_e2e"]',
      { timeout: 30000 }
    );
    ...
    await expect(page.getByText(/Quest Complete|E2E Strength Test/i).first()).toBeVisible({
      timeout: 15000,
    });
  ...
  test('should display workout HUD elements', async ({ page }) => {
    ...
    const _hudElements = page.locator(
      '.bg-black\\/40.backdrop-blur-xl, [data-testid="biometrics-hud"]'
    );
  ...
  test('should show exercise library when adding exercise', async ({ page }) => {
    ...
    const addExerciseBtn = page
      .locator('button:has-text("Add Exercise"), button:has([class*="Plus"]), [aria-label*="add"]')
      .first();
  ...
  test('should show progress chart toggle', async ({ page }) => {
    ...
    const chartToggle = page
      .locator(
        'button[title*="Progress"], button [class*="BarChart"], svg[class*="lucide-bar-chart"]'
      )
      .first();
    ...
    if (await chartToggle.isVisible({ timeout: 5000 })) {
      ...
      await expect(page.locator('.recharts-wrapper, [class*="AreaChart"]').first()).toBeVisible({
        timeout: 5000,
      });
    }
  });
...
test.describe('Iron Mines - Ghost Mode', () => {
  ...
  test('should limit visible events to MAX_VISIBLE_EVENTS', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="ghost-event-item"]')).toHaveCount(5, {
      timeout: 10000,
    });
  });
});
...
test.describe('Iron Mines - LiveSessionHUD Interactions', () => {
  ...
  test('should handle session leave action', async ({ page }) => {
    await page.evaluate(() => {
      (window as any).__mockCoOpSession = {
        id: 'test-session',
        participants: [{ id: 'me', userId: 'me', heroName: 'Hero', status: 'active' }],
        maxParticipants: 4,
      };
