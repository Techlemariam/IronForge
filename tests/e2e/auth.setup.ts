...
setup('authenticate', async ({ page }) => {
  ...
  await Promise.all([
    page
      ...
      .catch((e: any) => console.error('Navigation check failed:', e.message)),
  ...
  try {
    await Promise.race([
      ...
      page.waitForURL((url) => url.pathname === '/' || url.pathname === '/welcome', {
        timeout: 90000,
      }),
    ]);
    ...
    const onboardingVisible = await page
      .locator('h2:has-text("Awaken, Titan"), button:has-text("I Swear It")')
      .isVisible();
  ...
  if (await overlay.isVisible()) {
    ...
    await overlay
      ...
      .catch(() => console.log('Overlay did not disappear in time!'));
  }
