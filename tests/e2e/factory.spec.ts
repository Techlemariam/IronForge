...
test.describe('Factory Orchestration Dashboard', () => {
  ...
  test('should render all major orchestration sections', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Feature Assembly Line' }).first()).toBeVisible({
      timeout: 15000,
    });
  ...
  test('should Promote item from Backlog to Assembly Line', async ({ page }) => {
    ...
    const backlogSection = page
      .locator('[data-testid="backlog-section"], section:has(h2:has-text("Backlog"))')
      .first();
    await expect(backlogSection.getByText(itemTitle as string, { exact: true })).toHaveCount(0, {
      timeout: 15000,
    });
  });
