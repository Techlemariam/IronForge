...
test.describe('Oracle Chat', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const skipButton = page.locator(
      'button:has-text("Skip"), button:has-text("Close"), button:has-text("Later"), button:has-text("Continue")'
    );
  ...
  test('should display loading state while generating response', async ({ page }) => {
    ...
    const chatTrigger = page.locator(
      '[data-testid="oracle-chat-trigger"], button:has(.lucide-message-circle)'
    );
    ...
    if ((await inputField.count()) > 0) {
      ...
      const loadingIndicator = page.locator(
        '[class*="loading"], [class*="spinner"], .animate-spin, :has-text("thinking")'
      );
