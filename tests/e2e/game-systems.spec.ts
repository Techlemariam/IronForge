...
test.describe('Game Systems', () => {
  test('should load the Armory page', async ({ page }) => {
    ...
    await expect(page.getByText(/Armory|Scanning Inventory|Equipment/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
  ...
  test('should load the Colosseum page', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Iron Colosseum|Gladiators|Enter Arena|Find Match/i).first()
  ...
  test('should load the Bestiary page', async ({ page }) => {
    ...
    await expect(page.getByText(/Bestiary|Enemies Defeated|Lore Unlocked/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
  ...
  test('should load the World Map page', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Known World|Scouting Terrain|Return to Citadel/i).first()
    ).toBeVisible({ timeout: 15000 });
  });
});
