  ...
  describe('GET Verification', () => {
    it('should verify with correct token', async () => {
      ...
      const req = new NextRequest(
        'http://localhost/api/webhooks/strava?hub.mode=subscribe&hub.verify_token=TEST_TOKEN&hub.challenge=CHALLENGE_CODE'
      );
    ...
    it('should fail with incorrect token', async () => {
      const { GET } = await getHandlers();
      const req = new NextRequest(
        'http://localhost/api/webhooks/strava?hub.mode=subscribe&hub.verify_token=WRONG_TOKEN&hub.challenge=CHALLENGE_CODE'
      );
      const res = await GET(req);
