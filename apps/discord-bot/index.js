...
const { DISCORD_BOT_TOKEN, DISCORD_CLIENT_ID, N8N_WEBHOOK_URL, REMOTE_TRIGGER_SECRET } =
  process.env;
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('branch').setDescription('Target branch (default: main)').setRequired(false)
    ),
  new SlashCommandBuilder()
...
client.once('ready', () => {
  ...
  console.log(
    `📡 Webhook URL: ${N8N_WEBHOOK_URL || 'https://ironforge-coolify.tailafb692.ts.net/webhook/ironforge-trigger'}`
  );
});
