...
- **IronForge**: Next.js, PostgreSQL (Prisma), Cypress, Playwright, Doppler CLI, Snyk CLI, Docker, jq.
...
4. **Verktyg & Säkerhet**: Installera `doppler` CLI (för secrets-injektion), `jq` (för JSON-manipulering och metrics) samt `snyk` CLI (för säkerhetsskanning).
