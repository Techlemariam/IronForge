...
function analyzeFile(filePath: string, fixMode: boolean, knownSkills: Set<string>): Issue[] {
  ...
  const _codeBlockOpen = false;
  const _insideBash = false;
  const _justSawTurbo = false;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      const isSafe =
        blockContent.match(/^(ls|dir|echo|Get-Date|Write-Host|npx|npm|pnpm|git status)/m) &&
        !blockContent.includes('rm ') &&
        !blockContent.includes('sudo ');
...
function main() {
  ...
  console.log(
    `🔍 Analyzing ${files.length} agent workflows against ${knownSkills.size} skills...\n`
  );
