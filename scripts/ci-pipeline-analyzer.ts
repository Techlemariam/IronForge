...
function analyzeDuplicateSteps(content: string): Optimization[] {
  ...
  if (installCount > 2) {
    optimizations.push({
      ...
      suggestedFix:
        'Create composite action: checkout → pnpm → node → install → prisma generate. Reuse across jobs.',
  ...
  if (prismaGenCount > 2) {
    optimizations.push({
      ...
      suggestedFix:
        'Generate Prisma client once in first job, upload as artifact, download in other jobs.',
...
function analyzeTiming(content: string): Optimization[] {
  ...
  if (sleepMatches.length > 0) {
    const totalSleep = sleepMatches.reduce(
      (sum, s) => sum + Number.parseInt(s.split(' ')[1], 10),
      0
    );
    optimizations.push({
...
function analyzeSecurity(content: string): Optimization[] {
  ...
  if (unpinnedActions.length > 0 && hashPinned.length === 0) {
    optimizations.push({
      ...
      description:
        'Tag-based versions (e.g., @v4) can be moved. SHA-pinning prevents supply chain attacks.',
