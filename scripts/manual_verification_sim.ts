...
import type { TrainingPath } from '../src/types/training';
...
async function runSimulation() {
  ...
  if (modifiers.attackPower === 0.9 && modifiers.stamina === 1.3) {
  ...
  const mockWellness = {
    ctl: 50,
    atl: 50,
    tsb: 5, // Positive TSB = Performance Mode
    sleepScore: 85,
    hrv: 60,
    bodyBattery: 80,
    vo2max: 55,
  ...
  if (
    recommendation.rationale.includes('Pathfinder Path') ||
    recommendation.generatedSession?.name?.toLowerCase().includes('run') ||
    recommendation.title.includes('PATHFINDER')
  ) {
