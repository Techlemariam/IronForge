...
const REPORT_PATH = path.resolve(process.cwd(), '.agent/reports/ci-metrics.json');
...
function calculateMTTR(runs: CIRun[]): number | null {
  ...
  for (let i = 0; i < completed.length - 1; i++) {
    if (completed[i].conclusion === 'success' && completed[i + 1].conclusion === 'failure') {
...
function buildReport(runs: CIRun[]): MetricsReport {
  ...
  return {
    ...
    failureRate: completed.length > 0 ? Math.round((failures.length / completed.length) * 100) : 0,
...
console.log(`  MTTR: ${report.mttrMinutes !== null ? `${report.mttrMinutes} min` : 'N/A'}`);
