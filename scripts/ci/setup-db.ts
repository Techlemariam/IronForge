...
async function connectWithRetry(connectionString: string): Promise<pg.Client> {
  ...
  while (retries > 0) {
    try {
      ...
      retries--;
      console.warn(
        `⚠️ Connection failed. Retries left: ${retries}. Error: ${err instanceof Error ? err.message : String(err)}`
      );
      if (client) {
...
async function prepareDatabase(client: pg.Client, dbName: string) {
  ...
  try {
    ...
    console.warn(
      `⚠️ Standard CREATE DATABASE failed, attempting with C collation. Error: ${createErr instanceof Error ? createErr.message : String(createErr)}`
    );
...
async function setupDatabase() {
  ...
  const connectionString =
    process.env.ADMIN_DATABASE_URL || 'postgresql://postgres:password@127.0.0.1:5432/postgres';
