...
const PROTOCOLS: Record<string, RepairProtocol> = {
  RUN_TYPE_CHECK: {
    ...
    execute: () =>
      timedExec('RUN_TYPE_CHECK', 'LOW', 'pnpm check-types', () => safeExec('pnpm check-types')),
  ...
  RUN_LINT_FIX: {
    ...
    execute: () =>
      timedExec('RUN_LINT_FIX', 'LOW', 'pnpm lint --fix', () => safeExec('pnpm lint --fix')),
  ...
  REGENERATE_PRISMA: {
    ...
    execute: () =>
      timedExec('REGENERATE_PRISMA', 'MEDIUM', 'npx prisma generate', () => {
  ...
  FIX_MIGRATION_DRIFT: {
    ...
    execute: () =>
      timedExec('FIX_MIGRATION_DRIFT', 'MEDIUM', 'prisma migrate dev', () =>
  ...
  PUSH_SCHEMA: {
    ...
    execute: () =>
      timedExec('PUSH_SCHEMA', 'MEDIUM', 'prisma db push', () =>
  ...
  FIX_PG_USER: {
    ...
    execute: () =>
      timedExec('FIX_PG_USER', 'LOW', 'Fix pg_isready -U postgres', () => {
  ...
  ADD_SERVICE_WAIT: {
    ...
    execute: () =>
      timedExec('ADD_SERVICE_WAIT', 'LOW', 'Check service wait times', () => {
        return {
          success: true,
          output: 'Recommend: Replace static "sleep" with pg_isready loop in CI workflows.',
        };
      }),
    verify: () => true,
  },
  ...
  UPDATE_SNAPSHOTS: {
    ...
    execute: () =>
      timedExec('UPDATE_SNAPSHOTS', 'LOW', 'pnpm test -- -u', () => safeExec('pnpm test -- -u')),
  ...
  FIX_PATH_SEPARATORS: {
    ...
    execute: () =>
      timedExec('FIX_PATH_SEPARATORS', 'LOW', 'Scan for backslash paths', () => {
  ...
  FIX_ALIAS_CONFIG: {
    ...
    execute: () =>
      timedExec('FIX_ALIAS_CONFIG', 'LOW', 'Check Storybook config', () => {
  ...
  GENERATE_TESTS: {
    ...
    execute: () =>
      timedExec('GENERATE_TESTS', 'LOW', 'Analyze coverage gaps', () => {
  ...
  GENERATE_DOCS: {
    ...
    execute: () =>
      timedExec('GENERATE_DOCS', 'LOW', 'Scan for missing JSDoc', () => {
        return {
          success: true,
          output: 'Documentation generation requires /doc-generator workflow.',
        };
      }),
    verify: () => true,
  },
  ...
  ROTATE_SECRETS: {
    ...
    execute: () =>
      timedExec('ROTATE_SECRETS', 'HIGH', 'Scan for exposed secrets', () => {
        const result = safeExec(
          'git grep -rn "password\\|secret\\|api.key" -- "*.yml" "*.env*" "*.json" | grep -v node_modules | head -20'
        );
  ...
  SANITIZE_INPUT: {
    ...
    execute: () =>
      timedExec('SANITIZE_INPUT', 'HIGH', 'Flag for /red-team review', () => {
  ...
  CHECK_COOLIFY: {
    ...
    execute: () =>
      timedExec('CHECK_COOLIFY', 'HIGH', 'Check Coolify health', () => {
        return {
          success: true,
          output: 'Run /coolify-deploy to diagnose. Check Coolify dashboard.',
        };
      }),
    verify: () => true,
  },
  ...
  DOCKER_SERVICE_RESTART: {
    ...
    execute: () =>
      timedExec('DOCKER_SERVICE_RESTART', 'MEDIUM', 'manage-ci-services.ps1 -Action up', () =>
        safeExec(
          'powershell.exe -ExecutionPolicy Bypass -File scripts/ci/manage-ci-services.ps1 -Action up'
        )
      ),
    verify: () => {
      const result = safeExec(
        'docker ps --filter "name=ironforge-pg-l1" --filter "status=running" --format "{{.Names}}"'
      );
  ...
  DOCKER_SERVICE_RESET: {
    ...
    execute: () =>
      timedExec('DOCKER_SERVICE_RESET', 'MEDIUM', 'manage-ci-services.ps1 -Action reset', () =>
        safeExec(
          'powershell.exe -ExecutionPolicy Bypass -File scripts/ci/manage-ci-services.ps1 -Action reset'
        )
      ),
    verify: () => {
      const result = safeExec(
  ...
  DOCKER_RESOURCE_LIMIT: {
    ...
    execute: () =>
      timedExec('DOCKER_RESOURCE_LIMIT', 'HIGH', 'docker system prune', () => {
        return {
          success: true,
          output:
            'Recommendation: Run "docker system prune -f" and "docker volume prune -f" on host.',
        };
      }),
    verify: () => true,
  },
  ...
  MOVE_MOCK_TO_INIT_SCRIPT: {
    ...
    execute: () =>
      timedExec('MOVE_MOCK_TO_INIT_SCRIPT', 'MEDIUM', 'Scan test files', () => {
  ...
  ADD_DATA_TESTID: {
    ...
    execute: () =>
      timedExec('ADD_DATA_TESTID', 'MEDIUM', 'Identify missing testids', () => {
        return {
          success: true,
          output: 'Check E2E test logs for specific selector that timed out.',
        };
      }),
    verify: () => true,
  },
  ...
  ADD_NETWORK_MOCK: {
    ...
    execute: () =>
      timedExec('ADD_NETWORK_MOCK', 'MEDIUM', 'Detect unmocked requests', () => {
        return {
          success: true,
          output: 'Check test logs for ECONNREFUSED targets. Add route mocks.',
        };
      }),
    verify: () => true,
  },
  ...
  FIX_MOCK_REGISTRY: {
    ...
    execute: () =>
      timedExec('FIX_MOCK_REGISTRY', 'LOW', 'Check mock registry', () => {
        return {
          success: true,
          output: 'Verify tests/mocks/registry.ts matches component expectations.',
        };
      }),
    verify: () => true,
  },
  ...
  FIX_ASSERTION: {
    ...
    execute: () =>
      timedExec('FIX_ASSERTION', 'MEDIUM', 'Analyze assertion', () => {
  ...
  VALIDATE_ZOD_SCHEMA: {
    ...
    execute: () =>
      timedExec('VALIDATE_ZOD_SCHEMA', 'MEDIUM', '/zod-schema-validator', () => {
        return {
          success: true,
          output: 'Run /zod-schema-validator to compare schemas with API shape.',
        };
      }),
    verify: () => true,
  },
  ...
  EXTRACT_SHARED_CODE: {
    ...
    execute: () =>
      timedExec('EXTRACT_SHARED_CODE', 'LOW', 'Detect duplicates', () => {
  ...
  FIX_STATIC_ANALYSIS: {
    ...
    execute: () =>
      timedExec('FIX_STATIC_ANALYSIS', 'MEDIUM', 'Run Qodana', () => {
  ...
  ANALYZE_PERFORMANCE: {
    ...
    execute: () =>
      timedExec('ANALYZE_PERFORMANCE', 'MEDIUM', '/perf', () => {
  ...
  ANALYZE_BUNDLE: {
    ...
    execute: () =>
      timedExec('ANALYZE_BUNDLE', 'MEDIUM', '/bundle-analyzer', () => {
  ...
  FIX_VISUAL_REGRESSION: {
    ...
    execute: () =>
      timedExec('FIX_VISUAL_REGRESSION', 'MEDIUM', '/storybook-bridge', () => {
        return {
          success: true,
          output: 'Review Chromatic visual diffs. Accept or fix component changes.',
        };
      }),
    verify: () => true,
  },
  ...
  FIX_ACCESSIBILITY: {
    ...
    execute: () =>
      timedExec('FIX_ACCESSIBILITY', 'MEDIUM', '/a11y-auditor', () => {
  ...
  RESOLVE_CONFLICTS: {
    ...
    execute: () =>
      timedExec('RESOLVE_CONFLICTS', 'MEDIUM', 'git status', () => safeExec('git status')),
  ...
  VALIDATE_BIO_SYNC: {
    ...
    execute: () =>
      timedExec('VALIDATE_BIO_SYNC', 'MEDIUM', '/bio-validator', () => {
        return {
          success: true,
          output: 'Run /bio-validator to check Intervals/Hevy/Strava connections.',
        };
      }),
    verify: () => true,
  },
  ...
  MANUAL_INSPECTION: {
    ...
    execute: () =>
      timedExec('MANUAL_INSPECTION', 'HIGH', 'Manual review', () => {
        return {
          success: true,
          output: 'Error could not be classified. Manual inspection required.',
        };
      }),
    verify: () => true,
  },
...
function main() {
  ...
  if (filteredArgs.length === 0 && !args.includes('--stdin')) {
    console.log(
      'Usage: npx tsx scripts/repair-protocols.ts --classifications <json-file> [--json] [--all]'
    );
    console.log(
      '       echo "[...]" | npx tsx scripts/repair-protocols.ts --stdin [--json] [--all]'
    );
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    reportFile,
    JSON.stringify({ timestamp: new Date().toISOString(), classifications, results }, null, 2)
  );
