...
const REPORT_PATH = path.resolve(process.cwd(), '.agent/reports/test-stability.json');
...
function updateReport(results: TestResult[]): StabilityReport {
  ...
  report.flakyTests = Object.values(report.tests).filter((t) => t.isFlaky).length;
...
if (playwrightIdx !== -1 && args[playwrightIdx + 1]) {
  ...
  allResults = allResults.concat(parsePlaywrightResults(args[playwrightIdx + 1]));
}
