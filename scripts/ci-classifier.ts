...
const ERROR_PATTERNS: Record<string, ErrorPattern> = {
  ...
  RACE_CONDITION: {
    pattern:
      /mock.*after.*mount|useEffect.*async|addInitScript.*after.*goto|__mock.*undefined.*render/i,
  ...
  SELECTOR_TIMEOUT: {
    pattern:
      /timeout.*waiting.*selector|locator.*toBeVisible.*timeout|expect.*toBeVisible.*timeout/i,
  ...
  SERVICE_RACE_CONDITION: {
    ...
    solution:
      'Add health-check wait loop before prisma commands. Increase sleep or use pg_isready.',
    autoFixable: true,
    risk: 'LOW',
  ...
  PERF_DEGRADATION: {
    pattern:
      /lighthouse.*below|performance.*budget|LCP.*exceeded|CLS.*exceeded|bundle.*size.*exceeded/i,
  ...
  DOCKER_SERVICE_DOWN: {
    pattern:
      /Cannot connect to the Docker daemon|docker.*not running|Cannot connect to host.docker.internal/i,
