...
async function runSequential() {
    ...
    for (const duel of expiredDuels) {
        ...
        await calculateDuelScoreMock(); awaitPoints++;
        await calculateDuelScoreMock(); awaitPoints++;

        // 3. Update duel
        await mockDbCall(); awaitPoints++;

        // 4. Import service (simulated)
        await mockDbCall(); awaitPoints++;

        // 5. Winner rewards calculation
        await calculateRewardsMock(); awaitPoints++;
...
async function main() {
    ...
    const improvement = ((seqResult.duration - parResult.duration) / seqResult.duration * 100).toFixed(2);
