...
function parseCodeFactor(comments: Comment[]): Partial<IntelligenceResult> {
  ...
  for (const comment of comments) {
    if (comment.author.login !== 'codefactor' && !comment.author.login.includes('codefactor'))
      continue;
...
function parseDependabot(comments: Comment[]): Partial<IntelligenceResult> {
  ...
  for (const comment of comments) {
    if (comment.author.login !== 'dependabot[bot]' && !comment.author.login.includes('dependabot'))
      continue;
