 ...
 */
export function getTilesFromGpsTrack(points: Array<{ lat: number; lng: number }>): string[] {
 ...
 */
export function getDistanceBetweenTiles(tileA: string, tileB: string): number {
  const coordsA = tileIdToCoords(tileA);
  const coordsB = tileIdToCoords(tileB);
  return greatCircleDistance([coordsA.lat, coordsA.lng], [coordsB.lat, coordsB.lng], UNITS.m);
}
...
export function getTilesInRadius(
  ...
  return disk.filter((tileId) => isWithinHomeZone(tileId, centerLat, centerLng, radiusMeters));
}
 ...
 */
export function countConnectedTiles(ownedTileIds: Set<string>, startTileId: string): number {
