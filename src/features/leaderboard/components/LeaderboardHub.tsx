...
import type { LeaderboardEntry, LeaderboardScope } from '@/features/leaderboard/types';
import { motion } from 'framer-motion';
import { Crown, Globe, MapPin, Swords, Trophy, Users } from 'lucide-react';
...
export const LeaderboardHub: React.FC<LeaderboardHubProps> = ({
  ...
  const alliancePercentage =
    totalXp > 0 ? ((factionStats?.alliance?.totalXp || 0) / totalXp) * 100 : 50;
    ...
    <div className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden shadow-2xl animate-fade-in">
      ...
      <div className="bg-zinc-900/50 p-4 border-b border-zinc-800">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-heading text-rarity-gold uppercase tracking-widest flex items-center gap-2">
            <Crown className="w-6 h-6" />{' '}
            {activeTab === 'PVP' ? 'Champions of the Arena' : 'Iron Network Rankings'}
          </h3>
          <div className="text-xs font-mono text-zinc-500 uppercase tracking-tighter">
        ...
        <div className="flex gap-2 mb-4" role="tablist" aria-label="Leaderboard categories">
          <button
            ...
            className={`flex-1 px-4 py-3 rounded font-bold text-xs uppercase tracking-widest transition-all ${
              activeTab === 'PVP'
                ? 'bg-rarity-gold text-black shadow-glow-gold'
        ...
        {/* Filters */}
        <div
          className="flex gap-2 text-[10px] font-black uppercase tracking-widest"
          role="group"
