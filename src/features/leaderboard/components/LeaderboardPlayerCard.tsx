...
import { MapPin, Shield, Skull, Swords, Trophy, Zap } from 'lucide-react';
  ...
  return (
    <div
      className={`flex items-center justify-between p-4 hover:bg-zinc-900/40 transition-colors ${
        isMe ? 'bg-blue-900/10 border-l-2 border-blue-500' : ''
      }`}
    >
      <div className="flex items-center gap-4">
        <div className={`text-2xl font-black italic ${rankColor} w-8 text-center`}>{rank}</div>
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Faction Badge */}
            <span
              className={`text-[10px] px-2 py-0.5 rounded border uppercase tracking-widest font-black ${
            ...
            {showPvpStats && pvpRank && rankTitle && (
              <span
                className={`text-[10px] px-2 py-0.5 rounded border uppercase tracking-widest font-black flex items-center gap-1 ${
                  player.faction === 'ALLIANCE'
                    ? 'bg-blue-900/30 text-blue-400 border-blue-900/50'
                    : 'bg-red-900/30 text-red-400 border-red-900/50'
                }`}
      ...
      <div className="text-right">
        ...
        {showPowerRating && (
          <div className="text-lg font-bold text-purple-400">{player.powerRating || 0}</div>
        )}
      </div>
    </div>
  );
