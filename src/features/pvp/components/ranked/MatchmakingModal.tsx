...
import { type RankedOpponent, findRankedOpponentAction } from '@/actions/pvp/ranked';
        ...
        <DialogFooter className="flex-col sm:flex-col gap-2">
          {status === 'IDLE' && (
            <Button
              size="lg"
              onClick={startSearch}
              className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold uppercase tracking-widest"
            >
              Find Match
            </Button>
          )}
          ...
          {status === 'FOUND' && (
            <div className="grid grid-cols-2 gap-3 w-full">
              <Button
                variant="outline"
                onClick={onClose}
                className="border-red-900 text-red-500 hover:bg-red-950/50 hover:text-red-400"
              >
                Decline
              </Button>
              <Button
                onClick={acceptMatch}
                className="bg-green-600 hover:bg-green-500 text-white font-bold uppercase shadow-lg shadow-green-900/20 animate-pulse"
              >
                Accept Duel
              </Button>
            </div>
