...
import type { UnifiedLeaderboardEntry } from '@/actions/pvp/leaderboards';
import type { PlayerRating } from '@/actions/pvp/ranked';
...
import type { PvpSeason } from '@/types/prisma';
import { Calendar, Crown, Info, Medal, Swords, Trophy } from 'lucide-react';
...
export function RankedArenaClient({
  season,
  playerRating,
  leaderboard,
  userId,
  faction,
}: RankedArenaClientProps) {
  ...
  const daysRemaining = Math.ceil(
    (new Date(season.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
  );
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-4 text-center md:text-left">
            ...
            <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white">
              Ranked{' '}
              <span className="text-amber-500 text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-600">
                Arena
              </span>
            </h1>
            <p className="max-w-xl text-zinc-400 text-lg">
              Compete in rated battles to climb the ladder, earn exclusive titles, and claim
              seasonal rewards. Glory awaits the strong.
            </p>
      ...
      <Tabs defaultValue="leaderboard" className="w-full">
        <TabsList className="bg-zinc-900 border border-zinc-800" aria-label="Arena Navigation">
          <TabsTrigger value="leaderboard" className="font-bold uppercase tracking-wider text-xs">
            Global Leaderboard
          </TabsTrigger>
          <TabsTrigger value="rewards" className="font-bold uppercase tracking-wider text-xs">
            Season Rewards
          </TabsTrigger>
        </TabsList>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-zinc-800">
                  {leaderboard.map((entry, index) => (
                    <tr
                      key={index}
                      className={`hover:bg-zinc-800/50 transition-colors ${entry.userId === userId ? 'bg-amber-950/20' : ''}`}
                    >
                      <td className="px-6 py-4 font-mono text-zinc-400">
                        {index < 3 ? (
                          <span
                            className={`flex items-center gap-2 ${index === 0 ? 'text-amber-400' : index === 1 ? 'text-zinc-300' : 'text-orange-700'}`}
                          >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-zinc-500">
                          {entry.metadata?.title || 'Novice'}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <RankBadge
                          rating={entry.score}
                          faction={(entry.faction as Faction) || 'HORDE'}
                          size="sm"
                        />
                      </td>
                      <td className="px-6 py-4 text-right font-mono text-zinc-400">
                        {entry.metadata?.wins + entry.metadata?.losses > 0
                          ? Math.round(
                              (entry.metadata?.wins /
                                (entry.metadata?.wins + entry.metadata?.losses)) *
                                100
                            )
                          : 0}
                        %
