...
import type { ExtendedDuel } from '@/app/iron-arena/ArenaClient'; // Reusing type
...
export function DuelVictoryScreen({ duel, currentUserId, onClose }: DuelVictoryScreenProps) {
  ...
  const myStats =
    duel.challengerId === currentUserId
      ? duel.challengerDistance || duel.challengerScore
      : duel.defenderDistance || duel.defenderScore;
  const opponentStats =
    duel.challengerId === currentUserId
      ? duel.defenderDistance || duel.defenderScore
      : duel.challengerDistance || duel.challengerScore;
  ...
  const formatValue = (val: number) =>
    duel.duelType !== 'TITAN_VS_TITAN' ? (val / 1000).toFixed(2) : val;
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto min-h-[500px] flex items-center justify-center overflow-hidden rounded-3xl bg-slate-950 border border-slate-800 shadow-2xl">
      {/* Ambient Background */}
      <div
        className={`absolute inset-0 opacity-20 ${isWinner ? 'bg-amber-600' : 'bg-slate-800'}`}
      ...
      <div className="relative z-10 flex flex-col items-center w-full p-8 text-center space-y-8">
        ...
        >
          ...
          <h1
            className={`text-4xl md:text-5xl font-black uppercase tracking-tighter ${
              isWinner
                ? 'text-transparent bg-clip-text bg-gradient-to-b from-amber-300 to-amber-600'
                : 'text-slate-400'
            }`}
          >
        ...
        <div className="flex items-center justify-center gap-8 w-full max-w-lg">
          ...
          <div className="flex flex-col items-center gap-2">
            <Avatar
              className={`w-20 h-20 border-4 ${isWinner ? 'border-amber-500' : 'border-slate-700'}`}
            >
          ...
          <div className="flex flex-col items-center gap-2 opacity-80">
            ...
            <span className="text-xl font-bold font-mono text-slate-400">
              {formatValue(opponentStats)}{' '}
              <span className="text-xs text-slate-500">{metricUnit}</span>
            </span>
          </div>
        </div>
