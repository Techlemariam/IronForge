...
import { Bike, Footprints, Timer, TrendingUp, Trophy } from 'lucide-react';
...
type DuelType = 'TITAN_VS_TITAN' | 'DISTANCE_RACE' | 'SPEED_DEMON' | 'ELEVATION_GRIND';
type ActivityType = 'CYCLING' | 'RUNNING';

export function CardioDuelModal({ isOpen, onClose, opponentId }: CardioDuelModalProps) {
    ...
    try {
      const result = await createDuelChallengeAction({
        ...
        options: {
          ...
          activityType,
          durationMinutes:
            duelType === 'DISTANCE_RACE' || duelType === 'ELEVATION_GRIND' ? duration : undefined,
            ...
            >
              ...
              {duelType === 'SPEED_DEMON' && (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    {(activityType === 'RUNNING' ? [1, 3, 5, 10] : [5, 10, 20, 40]).map((d) => (
                      <button
                        key={d}
              ...
              {activityType === 'CYCLING' && (
                <div className="space-y-2 pt-2 border-t border-slate-800">
                  ...
                  <input
                    ...
                    step="0.1"
                    value={wkgTier}
                    onChange={(e) => setWkgTier(Number.parseFloat(e.target.value))}
