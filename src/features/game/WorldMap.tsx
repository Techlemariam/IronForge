...
import { getWorldStateAction, getRegionBossAction, WorldRegion } from '@/actions/world';
...
export default function WorldMap({ onClose, onEnterCombat }: WorldMapProps) {
    ...
    if (loading) return <div className="h-full flex items-center justify-center text-zinc-500 animate-pulse">Scouting Terrain...</div>;
    ...
    return (
        <div className="relative w-full h-full bg-black overflow-hidden select-none">
            ...
            <div className="absolute top-6 left-6 z-10 pointer-events-none">
                <h2 className="text-4xl font-black italic uppercase text-zinc-800 tracking-tighter">Known World</h2>
            ...
            <div className="relative w-full h-full">
                {regions.map((region) => (
                    ...
                    >
                        ...
                        {region.isUnlocked && (
                            <div className={`absolute inset-0 rounded-full animate-ping opacity-20 ${region.id === 'iron_forge' ? 'bg-orange-500' :
                                region.id === 'shadow_realms' ? 'bg-purple-500' : 'bg-white'
                                }`}></div>
                        )}
                        ...
                        <div className={`
                            ...
                            ${region.isUnlocked
                                ? region.id === 'iron_forge' ? 'bg-zinc-900 border-orange-500 text-orange-500 hover:shadow-[0_0_30px_rgba(249,115,22,0.6)]' :
                                    region.id === 'shadow_realms' ? 'bg-zinc-900 border-purple-500 text-purple-500 hover:shadow-[0_0_30px_rgba(168,85,247,0.6)]' :
                        ...
                        `}>
                            {region.isUnlocked ? <MapIcon className="w-6 h-6" /> : <Lock className="w-5 h-5" />}
                        </div>
                ...
                {selectedRegion && (
                    ...
                    >
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="text-2xl font-black italic uppercase text-[#ffd700]">{selectedRegion.name}</h3>
                                <div className="text-xs font-mono text-zinc-500 uppercase tracking-widest">
                                    Difficulty: {selectedRegion.levelReq >= 20 ? 'EXTREME' : selectedRegion.levelReq >= 10 ? 'HARD' : 'NORMAL'}
                                </div>
                            </div>
                            <button onClick={() => setSelectedRegion(null)} className="text-zinc-500 hover:text-white">✕</button>
                        </div>
