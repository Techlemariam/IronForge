...
import { MessageSquare, Swords, Timer, Zap } from 'lucide-react';
...
export function DuelArena({ duelId, currentUserId, onClose }: DuelArenaProps) {
  ...
  const { broadcastEmote } = useBroadcastEmote(
    duelId,
    duelState?.challenger?.id === currentUserId
      ? duelState?.challenger?.heroName
      : duelState?.defender?.heroName
  );
  ...
  const handleAttack = async () => {
    ...
    try {
      ...
      if (result.success && result.combatLog) {
        ...
        toast.success(
          `You dealt ${result.damageDealt?.challenger || result.damageDealt?.defender || 0} damage!`
        );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl bg-slate-900 border border-slate-700 rounded-2xl overflow-hidden shadow-2xl relative">
        ...
        <div
          className={`absolute inset-0 bg-[url('/assets/arena_bg.jpg')] bg-cover bg-center opacity-20`}
        />

        <div className="relative z-0 p-8 space-y-8">
          {/* Emote Display */}
          <EmoteOverlay matchId={duelId} currentUserId={currentUserId} isMuted={isMuted} />
          ...
          <div className="text-center space-y-2">
            <Badge
              variant="outline"
              className="bg-red-500/10 text-red-500 border-red-500/50 px-4 py-1 text-sm tracking-widest uppercase"
            >
