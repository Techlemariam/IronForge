...
export const StrengthLog: React.FC<ExerciseLogProps> = ({ userId, exerciseId, exerciseName, initialSets = [] }) => {
    const [sets, setSets] = useState<SetData[]>(initialSets);
    const { toast } = useToast();

    const addSet = () => {
        // Copy previous set values for convenience
        const previousSet = sets[sets.length - 1] || { reps: 10, weight: 20, rpe: 8 };
