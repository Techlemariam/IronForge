    ...
    return (
        <div className="bg-black/30 backdrop-blur-md rounded-xl p-4 border border-white/5 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full justify-center">
                <button onClick={() => setIsActive(!isActive)} className="p-3 bg-magma text-black rounded-full hover:bg-magma/80 transition-colors">
                    {isActive ? <Pause className="w-5 h-5 fill-current" /> : <Play className="w-5 h-5 fill-current ml-0.5" />}
                </button>
                <button onClick={() => { setIsActive(false); setTimeLeft(initialSeconds); }} className="p-3 bg-white/5 text-zinc-400 rounded-full hover:text-white transition-colors">
                    <RotateCcw className="w-5 h-5" />
                </button>
            </div>

            <div className="flex gap-2">
                <button onClick={() => addTime(30)} className="px-3 py-1 bg-white/5 rounded-md text-xs font-bold text-zinc-400 hover:text-white">+30s</button>
                <button onClick={() => addTime(-30)} className="px-3 py-1 bg-white/5 rounded-md text-xs font-bold text-zinc-400 hover:text-white">-30s</button>
            </div>
        </div>
    );
