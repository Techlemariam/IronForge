import { TITAN_THOUGHTS, type TitanThoughtCategory } from '@/data/titanThoughts';
...
export const useTitanReaction = ({
  ...
  useEffect(() => {
    ...
    const isUrgentContext = ['SPRINT', 'GRIND', 'ZONE_5', 'VICTORY'].includes(category);
    ...
    const shouldUpdate =
      (category !== lastCategoryRef.current && (isUrgentContext || timeSinceLastChange > 5000)) || // Fast switch to urgent
  ...
  }, [heartRate, power, cadence, ftp, isPaused, hasFinished, maxHeartRate, duration]);
