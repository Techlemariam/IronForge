...
export function TitanXPBar({
  ...
  const containerBorder = isElite
    ? 'border-rarity-gold shadow-[0_0_10px_rgba(255,215,0,0.3)]'
    ...
    <div className="w-full font-serif relative">
      ...
      {showLevelUp && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md animate-fade-in-fast pointer-events-none">
          {/* Burst Background */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-rarity-gold/20 via-transparent to-transparent animate-pulse-slow" />

          {/* Icon */}
          <div className="relative mb-6 animate-slide-up-fade">
            <ArrowUpCircle className="w-24 h-24 text-rarity-gold drop-shadow-[0_0_30px_rgba(255,215,0,0.6)]" />
            <Sparkles className="absolute -top-4 -right-4 w-12 h-12 text-white animate-spin-slow" />
          </div>

          <h2 className="text-4xl md:text-6xl font-serif font-black text-transparent bg-clip-text bg-gradient-to-b from-rarity-gold to-armor uppercase tracking-[0.2em] mb-4 animate-scale-bounce drop-shadow-lg">
            Level Up
          </h2>
          ...
          <div className="mt-8 px-6 py-2 border border-rarity-gold/50 bg-rarity-gold/10 rounded-full text-rarity-gold text-sm uppercase tracking-widest font-bold animate-pulse">
      ...
      <div className="flex justify-between items-end mb-2 px-1">
        <div className="flex items-baseline gap-2">
          <span
            className={`text-xs font-bold tracking-widest uppercase ${isElite ? 'text-rarity-gold' : 'text-zinc-500'}`}
          >
            Level
          </span>
          <span
        ...
        <div className="text-right">
          <span
            className={`font-mono font-bold text-sm ${isElite ? 'text-white' : isRested ? 'text-blue-400' : 'text-rarity-gold'}`}
          >
            {animatedXP}
