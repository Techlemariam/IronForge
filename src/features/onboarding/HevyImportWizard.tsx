    ...
    if (complete) {
        return (
            <div className="text-center p-8 bg-zinc-900 border border-emerald-500/20 rounded-xl">
                ...
                <p className="text-zinc-400 mb-6">Your legendary history has been inscribed in the archives.</p>
                <button
                    onClick={() => { setFile(null); setStats(null); setComplete(false); }}
    ...
    return (
        <div className="space-y-6">
            {!file ? (
                <div
                    {...getRootProps()}
                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${isDragActive ? 'border-magma bg-magma/5' : 'border-zinc-700 hover:border-zinc-500 bg-zinc-900/50'
                        }`}
                >
                    <input {...getInputProps()} />
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                    ...
                    <div className="bg-yellow-500/10 border border-yellow-500/20 p-4 rounded-lg flex items-start gap-3 mb-6">
                        ...
                        <div className="text-sm text-yellow-200/80">
                            Missing exercises will be automatically created. Duplicates will be skipped based on date and exercise.
                        </div>
                    </div>
