...
import { Shield, Skull, Sparkles, Zap } from 'lucide-react';
import type React from 'react';
import type { BioBuff } from '../BioBuffService';
          ...
          <div className="flex gap-2 mt-1">
            {buff.effects.attackMod && (
              <span className="flex items-center gap-1">
                <Zap className="w-3 h-3" /> {buff.effects.attackMod > 1 ? '+' : ''}
                {Math.round((buff.effects.attackMod - 1) * 100)}% ATK
              </span>
            )}
            {buff.effects.defenseMod && (
              <span className="flex items-center gap-1">
                <Shield className="w-3 h-3" /> {buff.effects.defenseMod > 1 ? '+' : ''}
