...
export const BioBuffBadge: React.FC<BioBuffBadgeProps> = ({ buff, showDetails = false, className }) => {
    if (!buff) return null;
    ...
    return (
        <motion.div
            ...
            className={cn("rounded-lg border px-3 py-2 backdrop-blur-sm", colors, className)}
        >
            <div className="flex items-center gap-2">
                {isDebuff ? <Skull className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                <span className="font-bold text-sm uppercase tracking-wider">{buff.name}</span>
            </div>
                    ...
                    <div className="flex gap-2 mt-1">
                        {buff.effects.attackMod && (
                            <span className="flex items-center gap-1">
                                <Zap className="w-3 h-3" /> {(buff.effects.attackMod > 1 ? '+' : '')}{Math.round((buff.effects.attackMod - 1) * 100)}% ATK
                            </span>
                        )}
                        {buff.effects.defenseMod && (
                            <span className="flex items-center gap-1">
                                <Shield className="w-3 h-3" /> {(buff.effects.defenseMod > 1 ? '+' : '')}{Math.round((buff.effects.defenseMod - 1) * 100)}% DEF
                            </span>
                        )}
                    </div>
                </div>
