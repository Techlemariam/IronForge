...
export class BioBuffService {
    static calculateBuff(sleepScore: number, hrv: number, hrvBaseline: number): BioBuff {
        ...
        if (sleepScore >= 90 && hrvDeviation >= 20) return BUFF_DEFINITIONS.IRON_CONSTITUTION;
        if (sleepScore >= 80 && hrvDeviation >= 0) return BUFF_DEFINITIONS.WELL_RESTED;
