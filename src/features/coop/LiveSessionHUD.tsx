...
import { CoOpService, type CoOpSession } from '@/services/coop/CoOpService';
import { AnimatePresence, motion } from 'framer-motion';
import { UserPlus, Users, Wifi } from 'lucide-react';
  ...
  return (
    <div className="fixed top-4 right-4 z-50 flex flex-col items-end pointer-events-auto">
      ...
      <motion.button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-full backdrop-blur-md border border-white/10 transition-colors ${
          activeSession
                  ...
                  {activeSession.participants.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${p.status === 'active' ? 'bg-green-500' : 'bg-red-500'}`}
                        />
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto" data-testid="session-list">
                    {availableSessions.map((session) => (
                      <div
                        ...
                        tabIndex={0}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') handleJoin(session);
                        }}
                        className="flex items-center justify-between bg-white/5 p-2 rounded hover:bg-white/10 cursor-pointer"
