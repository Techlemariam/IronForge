...
import { Arena, Bestiary, EquipmentArmory, Grimoire, WorldMap } from './SecondaryViews';
import { TodaysMission } from './TodaysMission';

// Dynamic imports for heavy/client-only components
const RoutineSelector = dynamic(() => import('@/features/training/RoutineSelector'), {
  ssr: false,
});
...
export const ViewRouter: React.FC<ViewRouterProps> = ({
  ...
  switch (state.currentView) {
    ...
    case 'training_center':
      return (
        <TrainingCenter
          ...
          onImportRoutines={() => dispatch({ type: 'SET_VIEW', payload: 'import_routines' })}
        />
      );
        ...
        <div className="p-4 md:p-8 animate-fade-in relative min-h-screen">
          ...
          <RoutineSelector
            ...
            onSelectRoutine={async (routine) => {
              try {
                const { importHevyRoutineToTemplateAction } = await import(
                  '@/actions/integrations/hevy'
                );
    ...
    case 'iron_mines':
      console.log(
        '[ViewRouter] Mounting IronMines. ActiveQuest Exercises:',
        state.activeQuest?.length
      );
      return (
        <IronMines
    ...
    case 'item_shop':
    case 'marketplace':
      return <Marketplace onClose={() => dispatch({ type: 'SET_VIEW', payload: 'citadel' })} />;
    ...
    case 'forge':
      return <TheForge onClose={() => dispatch({ type: 'SET_VIEW', payload: 'citadel' })} />;
    ...
    case 'grimoire':
      return <Grimoire onClose={() => dispatch({ type: 'SET_VIEW', payload: 'citadel' })} />;

    case 'guild_hall':
      return (
        <ViewWithCloseButton onClose={() => dispatch({ type: 'SET_VIEW', payload: 'citadel' })}>
    ...
    case 'arena':
      return <Arena onClose={() => dispatch({ type: 'SET_VIEW', payload: 'citadel' })} />;
    ...
    default:
      return <Citadel state={state} dispatch={dispatch} titanState={titanState} />;
  }
};
