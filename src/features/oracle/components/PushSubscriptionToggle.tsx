...
export function PushSubscriptionToggle() {
  const { isSupported, subscription, subscribe, unsubscribe, loading, permission } =
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-void/50 border border-forge-700 rounded-lg">
      <div className="flex items-center gap-3">
        <div
          className={`p-2 rounded-full ${subscription ? 'bg-emerald-500/20 text-emerald-500' : 'bg-forge-700 text-forge-400'}`}
        >
          {subscription ? <Bell className="w-5 h-5" /> : <BellOff className="w-5 h-5" />}
        </div>
        <div>
          <h3 className="text-sm font-bold text-forge-100 uppercase tracking-tight">
            Oracle Comms Link
          </h3>
      ...
      <Button
        ...
        className={
          subscription
            ? 'border-forge-600 text-forge-300'
            : 'bg-magma hover:bg-magma-light text-white font-bold'
        }
      >
        {loading ? (
