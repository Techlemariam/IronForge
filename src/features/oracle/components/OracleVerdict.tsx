...
import { AlertTriangle, Flame, Moon, Sparkles, Zap } from 'lucide-react';
      ...
      >
        <div className="flex items-center gap-4 text-zinc-500">
          ...
          <div>
            ...
            <p className="text-sm">Connect wellness data to receive guidance.</p>
          </div>
        </div>
      </div>
    );
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className={cn('text-2xl font-black uppercase tracking-wide', verdict.color)}>
              {verdict.label}
            </h3>

            <p className="text-sm text-zinc-300 mt-1 truncate" title={decree.description}>
              {decree.description}
            </p>
          </div>

          {/* XP Multiplier Badge */}
          {decree.effect?.xpMultiplier && decree.effect.xpMultiplier !== 1.0 && (
            <div className="flex-shrink-0 text-center">
              <div
                className={cn(
                  'text-2xl font-black',
                  decree.effect.xpMultiplier > 1 ? 'text-green-400' : 'text-red-400'
                )}
              >
        ...
        <div className="mt-4 pt-3 border-t border-white/5">
          <p className="text-xs font-mono text-zinc-500 italic">{`"${decree.label}"`}</p>
        </div>
      </motion.div>
    </AnimatePresence>
  );
