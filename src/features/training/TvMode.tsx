...
import {
    Heart, Zap, Timer, Flame, Trophy, ChevronUp, ChevronDown,
    Bluetooth, BluetoothOff, Maximize, Minimize, X, Loader2
} from 'lucide-react';
...
const ZONE_COLORS: Record<number, { bg: string, border: string, text: string }> = {
    ...
    3: { bg: 'bg-green-950/30', border: 'border-green-900', text: 'text-green-400' },
    4: { bg: 'bg-yellow-950/30', border: 'border-yellow-900', text: 'text-yellow-400' },
...
export const TvMode: React.FC<TvModeProps> = ({
    ...
    const { lastEvent, broadcast, isConnected: isRelayConnected } = useCompanionRelay(relayRole, sessionId);
    ...
    // Guild Wars Hook
    const { raidState } = useGuildContribution({
        power: currentPower,
        heartRate: currentHr,
        zone,
        isEnabled: sessionStats.elapsedSeconds > 0 && !sensorsMenuOpen
    });
    ...
    return (
        ...
        >
            ...
            <div className="md:hidden flex-1 flex flex-col p-6">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <div className="flex gap-2">
                        <div className={cn("w-2 h-2 rounded-full", isHrConnected ? "bg-green-500" : "bg-red-500")} />
                        <div className={cn("w-2 h-2 rounded-full", isPowerConnected ? "bg-green-500" : "bg-red-500")} />
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-auto">
                    <div className="bg-zinc-900/80 p-6 rounded-2xl border border-zinc-800 flex flex-col items-center justify-center">
                        <Heart className={cn("w-8 h-8 mb-2", zone >= 4 ? "text-red-500 animate-pulse" : "text-zinc-500")} />
                        <span className="text-4xl font-black text-white">{currentHr}</span>
                        <span className="text-xs text-zinc-500 uppercase">BPM</span>
                    </div>
                    <div className="bg-zinc-900/80 p-6 rounded-2xl border border-zinc-800 flex flex-col items-center justify-center">
                        <Zap className={cn("w-8 h-8 mb-2", isPowerConnected ? "text-yellow-400" : "text-zinc-500")} />
                ...
                <div className="space-y-4">
                    <button
                        onClick={(e) => { e.stopPropagation(); setSensorsMenuOpen(true); }}
            ...
            <div className="hidden md:block relative w-full h-full">
                <div className={cn("absolute inset-0 pointer-events-none border-8", zoneStyle.border, "shadow-[inset_0_0_60px_10px]")} />
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-4">
                                    <Heart className={cn("w-12 h-12 animate-pulse", zone >= 4 ? "text-red-500" : "text-white")} />
                                    <div>
                                ...
                                <div className="flex items-center gap-2 pl-2">
                                    <div className={cn("px-3 py-1 rounded-full font-bold text-sm uppercase tracking-wider bg-black/50 backdrop-blur", zoneStyle.text)}>
                                        Zone {zone}
                                    </div>
                            ...
                            <div className="absolute top-8 left-1/2 -translate-x-1/2 max-w-2xl text-center">
                                <AnimatePresence mode="wait">
                                    {titanDialogue && (
                                        ...
                                        >
                                            <p className="text-xl font-medium text-white/90 italic">&quot;{titanDialogue}&quot;</p>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 items-end">
                                <div className="flex items-center gap-4">
                                    ...
                                    <Zap className={cn("w-12 h-12", isPowerConnected ? "text-yellow-400 fill-yellow-400" : "text-zinc-700")} />
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className={cn("p-2 rounded-lg transition-colors", qrVisible ? "bg-white text-black" : "bg-zinc-800 hover:bg-zinc-700 text-zinc-400")}
                                        >
                                        ...
                                        >
                                            {sensorsConnected ? <Bluetooth className="w-5 h-5 text-blue-400" /> : <BluetoothOff className="w-5 h-5 text-zinc-500" />}
                                        </button>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onExit(); }}
                                            className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"
                ...
                <AnimatePresence>
                    {hudVisible && raidState.active && (
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: 20 }}
                            className="absolute bottom-12 left-1/2 -translate-x-1/2 w-full max-w-4xl px-8"
                        >
                            <div className="bg-zinc-900/80 backdrop-blur border border-zinc-700 p-4 rounded-xl shadow-2xl relative overflow-hidden">
                                {/* Boss HP Bar */}
                                <div className="absolute inset-0 z-0">
                                    <div
                                        className="h-full bg-red-900/30 transition-all duration-1000 ease-out"
                                        style={{ width: `${(raidState.currentHp / raidState.totalHp) * 100}%` }}
                                    />
                                </div>
                                ...
                                <div className="relative z-10 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-xs text-red-400 font-bold uppercase tracking-wider">Guild Raid Target</div>
                                            <div className="text-xl font-black text-white">{raidState.bossName}</div>
                                            <div className="text-sm text-zinc-400">
                                                {raidState.currentHp.toLocaleString()} / {raidState.totalHp.toLocaleString()} HP
                                            </div>
                                        </div>
                                    </div>

                                    <div className="text-right">
                                        <div className="text-xs text-amber-400 font-bold uppercase tracking-wider">Your Contribution</div>
                                        <div className="text-2xl font-black text-white tabular-nums">
                                            +{raidState.mySessionDamage.toLocaleString()}
                                        </div>
