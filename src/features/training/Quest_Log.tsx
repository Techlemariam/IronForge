...
import BerserkerMode from '../../components/game/BerserkerMode';
import { saveQuestToHevy } from '../../services/hevy'; 
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#050505] font-serif overflow-y-auto overflow-x-hidden relative scroll-smooth">
      ...
      <div className={`transition-filter duration-500 ${isBerserkerActive ? 'blur-sm' : ''}`}>
        ...
        {activeExIndex === exercises.length - 1 && exercises[activeExIndex].completed && (
         <div className="text-center mt-10 pb-20">
            <button 
                onClick={onComplete} 
                className="bg-magma-DEFAULT hover:bg-magma-DEFAULT/80 text-white font-heading py-4 px-8 text-xl border-2 border-magma-glow shadow-neon-orange w-full md:w-auto"
            >
                MISSION COMPLETE - SAVE DATA
            </button>
         </div>
      )}
