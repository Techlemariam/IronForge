...
import type { ChaseDifficulty, ChaseState } from '@/types/chase';
...
import { type TrainingMetric, calculatePaceZone, calculatePowerZone } from './logic/zones';
          ...
          <div className="mt-4 flex gap-2">
            {(['easy', 'normal', 'hard', 'nightmare'] as ChaseDifficulty[]).map((d) => (
              <button
                ...
                className={`flex-1 py-1 text-xs font-bold uppercase rounded transition-colors ${
                  chaseDifficulty === d
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setTotalDistanceKm((prev) => {
        ...
        if (duel && (newDist - lastReportedDistance.current > 0.1 || currentDuration % 10 === 0)) {
          updateCardioDuelProgressAction({
            duelId: duel.id,
            distanceKm: newDist,
            durationMinutes: Math.floor(currentDuration / 60), // Optional but good for consistency
          }).catch((err) => console.error('Duel update failed', err));
    ...
    <div className="flex flex-col min-h-screen bg-zinc-950 text-white">
          ...
          {pocketCastsConnected && (
            <button
              ...
              className={`p-2 ml-2 rounded-lg transition-colors ${
                showPodcast ? 'bg-red-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:text-white'
              }`}
      ...
      <footer className="flex items-center justify-between px-6 py-2 bg-zinc-900 border-t border-zinc-800 text-xs text-zinc-500">
        <div className="flex items-center gap-4">
          ...
          {videoUrl && (
            <span>
              Video: <span className="text-zinc-400 truncate max-w-xs">{videoUrl}</span>
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
