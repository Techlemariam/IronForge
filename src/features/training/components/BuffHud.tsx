...
const BuffHud: React.FC<BuffHudProps> = ({ currentZone, rewards, durationSeconds, metric, currentValue, onValueChange }) => {
    ...
    return (
        <div className="absolute top-20 right-4 z-40 bg-zinc-900/90 border-2 border-zinc-700/50 rounded-xl p-4 w-64 shadow-2xl backdrop-blur-md">
            ...
            {/* Active Buff Display */}
            <div className={`flex items-center gap-3 mb-4 p-2 rounded-lg bg-black/40 border border-zinc-800/50`}>
            ...
            <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] text-zinc-500">
                    ...
                    <span className="font-mono text-zinc-300">{currentValue} {config.unit}</span>
                </div>
                <input
                    type="range"
                    min={config.min}
            ...
            <div className="mt-2 text-center">
                <span className={`text-xs font-bold font-mono px-2 py-0.5 rounded border ${activeBuff.color} border-current bg-black/50`}>
