...
import { MOBILITY_LAYER_BONUSES, RECOVERY_LAYER_BONUSES, PATH_INFO } from '@/data/builds';
import { PathSelector } from '@/components/PathSelector';
import { PassiveLayerProgress } from '@/components/PassiveLayerProgress';
import { ArrowLeft, Clock, Activity, Zap, Book, Dumbbell, Heart, Timer } from 'lucide-react';
    ...
    React.useEffect(() => {
        ...
        const loadData = async () => {
            const [acts, settings] = await Promise.all([
                ...
                import('@/actions/intervals').then(mod => mod.getActivitiesAction(
                    new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Last 7 days
                    new Date().toISOString().split('T')[0]
                )),
                import('@/actions/intervals').then(mod => mod.getAthleteSettingsAction())
            ]);
    ...
    return (
        <div className="w-full max-w-6xl mx-auto p-4 md:p-6 space-y-8 animate-fade-in pb-20">
            ...
            <div className={`relative overflow-hidden rounded-2xl border border-white/10 p-8 shadow-2xl`}>
                ...
                <div className={`absolute -right-20 -top-20 w-64 h-64 bg-${pathColor}-500/20 blur-[100px] rounded-full`} />
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        ...
                        <div className="flex gap-4">
                            <div className="bg-black/40 backdrop-blur-md p-3 rounded-xl border border-white/5 text-center min-w-[100px]">
                                ...
                                <div className={`text-xl font-black text-${pathColor}-400`}>{pathInfo.strengthLevel}</div>
                            </div>
                            <div className="bg-black/40 backdrop-blur-md p-3 rounded-xl border border-white/5 text-center min-w-[100px]">
                                <div className="text-xs text-zinc-500 uppercase font-bold">Cardio</div>
                                <div className={`text-xl font-black text-${pathColor}-400`}>{pathInfo.cardioLevel}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
const CodexTabs: React.FC<{ activePath: TrainingPath; onSelectWorkout: (w: WorkoutDefinition) => void }> = ({ activePath, onSelectWorkout }) => {
    ...
    const cardio = WORKOUT_LIBRARY.filter(w => (w.type === 'RUN' || w.type === 'BIKE') && !w.name.toLowerCase().includes('recovery'));
    const recovery = WORKOUT_LIBRARY.filter(w => w.name.toLowerCase().includes('recovery') || w.intensity === 'LOW');

    const getWorkoutsForTab = () => {
        switch (activeTab) {
            case 'FOR_YOU': return recommended;
        ...
        <section className="space-y-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex p-1 bg-zinc-900/80 backdrop-blur-md rounded-lg border border-white/5">
                    <TabButton active={activeTab === 'FOR_YOU'} onClick={() => setActiveTab('FOR_YOU')} icon={<Zap className="w-3 h-3" />} label="For You" />
                    <TabButton active={activeTab === 'STRENGTH'} onClick={() => setActiveTab('STRENGTH')} icon={<Dumbbell className="w-3 h-3" />} label="Strength" />
                    <TabButton active={activeTab === 'CARDIO'} onClick={() => setActiveTab('CARDIO')} icon={<Heart className="w-3 h-3" />} label="Cardio" />
                    <TabButton active={activeTab === 'RECOVERY'} onClick={() => setActiveTab('RECOVERY')} icon={<Activity className="w-3 h-3" />} label="Recovery" />
                </div>
            </div>
...
const HeroWorkoutCard: React.FC<{ workout: WorkoutDefinition; onClick: () => void }> = ({ workout, onClick }) => {
...
const CompactWorkoutCard: React.FC<{ workout: WorkoutDefinition; onClick: () => void }> = ({ workout, onClick }) => {
    return (
        <motion.div
            layout
        ...
        >
            <div className="flex flex-col">
                <h4 className="font-bold text-zinc-200 text-sm group-hover:text-magma transition-colors">{workout.name}</h4>
                <div className="flex items-center gap-2 text-[10px] text-zinc-500 mt-1">
                    <span className="bg-white/5 px-1.5 py-0.5 rounded text-zinc-400">{workout.durationMin}m</span>
