      ...
      <Tabs defaultValue="players" className="w-full">
          ...
          <div className="divide-y divide-emerald-500/10">
            {users.length === 0 ? (
              <div className="p-8 text-center text-gray-500">
                No data available yet. Start running!
              </div>
            ) : (
              users.map((user) => (
                <div
                  key={user.userId}
                  className="flex items-center p-4 hover:bg-emerald-500/5 transition-colors"
                >
                  <div className="w-8 flex justify-center font-bold text-gray-400 mr-4">
                    {user.rank <= 3 ? (
                      <Medal className={`w-6 h-6 ${getRankColor(user.rank)}`} />
                    ) : (
                      user.rank
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {user.guildTag && (
                        <Badge
                          variant="outline"
                          className="text-[10px] bg-blue-900/20 border-blue-800 text-blue-400 px-1 py-0 h-4 rounded"
                        >
                          [{user.guildTag}]
                        </Badge>
                      )}
        ...
        <TabsContent value="guilds" className="m-0">
          <div className="divide-y divide-emerald-500/10">
            {guilds.length === 0 ? (
              <div className="p-8 text-center text-gray-500">
                No guilds have claimed territory yet.
              </div>
            ) : (
              guilds.map((guild) => (
                <div
                ...
                >
                  <div className="w-8 flex justify-center font-bold text-gray-400 mr-4">
                    {guild.rank <= 3 ? (
                      <Medal className={`w-6 h-6 ${getRankColor(guild.rank)}`} />
                    ) : (
                      guild.rank
                    )}
                  </div>
