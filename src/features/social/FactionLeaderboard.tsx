...
export const FactionLeaderboard: React.FC<FactionLeaderboardProps> = ({
  stats,
  leaderboard,
  currentUserId,
  isLoading = false,
  ...
  const alliancePercentage =
    totalXp > 0 ? (stats.alliance.totalXp / totalXp) * 100 : 50;
    ...
    return (
      <div className="space-y-8 p-4 md:p-6 bg-forge-900 min-h-[50vh] text-white rounded-xl border border-white/5">
        <div className="text-center space-y-2">
          <h2 className="text-3xl font-bold uppercase tracking-widest text-magma">
            Faction War
          </h2>
        ...
        <div className="relative h-24 bg-black/50 rounded-full overflow-hidden border-2 border-white/10 flex">
          <Skeleton className="w-1/2 h-full rounded-none opacity-20" />
          <Skeleton className="w-1/2 h-full rounded-none opacity-20" />
        ...
        <div className="space-y-2">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="flex items-center gap-4 p-3 rounded border border-white/5 bg-black/20"
            >
      ...
      <div className="relative h-24 bg-black/50 rounded-full overflow-hidden border-2 border-white/10 flex">
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="text-2xl font-black italic text-white drop-shadow-lg">
              ALLIANCE
            </h3>
            <p className="font-mono text-xs opacity-80">
              {stats.alliance.totalXp.toLocaleString()} XP
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative z-10 text-right">
            ...
            <p className="font-mono text-xs opacity-80">
              {stats.horde.totalXp.toLocaleString()} XP
            </p>
          </div>
        </motion.div>
      ...
      <div className="bg-zinc-900/50 rounded-lg p-4 border border-white/5">
        ...
        <div className="space-y-2">
          {leaderboard.map((entry, idx) => (
            ...
            >
              <span
                className={`font-mono font-bold w-6 text-center ${idx < 3 ? "text-yellow-400" : "text-zinc-500"}`}
              >
                #{idx + 1}
              </span>
              ...
              <div className="flex-1">
                <h4
                  className={`font-bold text-sm ${entry.userId === currentUserId ? "text-magma" : "text-white"}`}
                >
                  {entry.heroName}
                  {entry.guildName && (
                    <span className="ml-2 text-[10px] text-zinc-500 font-mono tracking-tighter">
                      &lt;{entry.guildName}&gt;
                    </span>
                  )}
                </h4>
              ...
              <div className="text-right">
                <div
                  className={`text-xs font-bold px-2 py-0.5 rounded ${entry.faction === "ALLIANCE" ? "text-blue-400 bg-blue-900/20" : "text-red-400 bg-red-900/20"}`}
                >
                  {entry.faction}
                </div>
