...
import { MuscleGroupVolume, AuditReport, MuscleAudit, WeaknessLevel, BalanceRatio, RPVolumeStandards } from '../types/auditor';
...
export const auditWeaknesses = (volumes: MuscleGroupVolume[]): AuditReport => {
    ...
    const highestPriorityGap = muscleAudits
...
const analyzeMuscle = (vol: MuscleGroupVolume, standards: RPVolumeStandards): MuscleAudit => {
    ...
    if (["Side Delts", "Abs", "Back (Width)"].includes(vol.muscleGroup) && level !== WeaknessLevel.NONE) {
        priority += 10;
    }
