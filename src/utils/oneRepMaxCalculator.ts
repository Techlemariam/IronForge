...
export function calculateE1rm(weight: number, reps: number, formula: Formula = 'EPLEY'): number {
    switch (formula) {
        case 'EPLEY': return calculateE1rmEpley(weight, reps);
        case 'BRZYCKI': return calculateE1rmBrzycki(weight, reps);
        case 'LANDER': return calculateE1rmLander(weight, reps);
        case 'LOMBARDI': return calculateE1rmLombardi(weight, reps);
        case 'MAYHEW': return calculateE1rmMayhew(weight, reps);
        case 'OCONNER': return calculateE1rmOConner(weight, reps);
        case 'WATHAN': return calculateE1rmWathan(weight, reps);
        default: return calculateE1rmEpley(weight, reps);
    }
}
...
export function calculateE1rmAllFormulas(weight: number, reps: number): MultiFormulaResult {
    const formulas: Formula[] = ['EPLEY', 'BRZYCKI', 'LANDER', 'LOMBARDI', 'MAYHEW', 'OCONNER', 'WATHAN'];
 ...
 */
export function generateRepTable(e1rm: number): Array<{ reps: number; weight: number; percentage: number }> {
