...
import type { MuscleGroup, WorkoutDefinition } from '@/types/training';

// Phase Allocation Matrix
const PHASE_ALLOCATION: Record<
  MacroPhase,
  { strength: number; cardio: number; mobility: number; description: string }
> = {
  CARDIO_BUILD: {
    strength: 0.2,
    cardio: 0.7,
...
export class GoalPriorityEngine {
  ...
  static needsDeload(metrics: SystemMetrics): boolean {
    ...
    if (
      ...
      metrics.tsb === null ||
      metrics.tsb === undefined
    ) {
      return metrics.soreness > 7 || metrics.mood === 'EXHAUSTED' || metrics.sleepScore < 30;
    }
  ...
  private static getUpcomingDeadline(
    goals: WardensManifest['goals']
  ...
  private static getMuscleForExercise(exerciseIdOrName: string): MuscleGroup | null {
    // Try direct map
    const muscle = HEVY_EXERCISE_MAP[exerciseIdOrName];
