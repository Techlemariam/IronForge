...
import { CHASE_DIFFICULTY_PRESETS, type ChaseDifficulty, type ChaseState } from '@/types/chase';
...
export class ChaseEngine {
   ...
   */
  static initializeChase(monster: Monster, difficulty: ChaseDifficulty = 'normal'): ChaseState {
  ...
  static updateChase(
    ...
    const playerDistanceIncrement = (playerSpeedKph / 3600) * deltaSeconds * 1000;
    const chaserDistanceIncrement = (chaserEffectiveSpeed / 3600) * deltaSeconds * 1000;

    // Update distances
    const newPlayerDistance = state.playerDistanceMeters + playerDistanceIncrement;
    const newChaserDistance = state.chaserDistanceMeters + chaserDistanceIncrement;

    // Calculate new gap
    const newGap = state.distanceGapMeters + (playerDistanceIncrement - chaserDistanceIncrement);
   ...
   */
  static getRequiredPace(state: ChaseState, difficulty: ChaseDifficulty = 'normal'): number {
    const config = CHASE_DIFFICULTY_PRESETS[difficulty];
    const chaserBaseSpeed = state.chaser.chaseSpeedKph ?? 8;
