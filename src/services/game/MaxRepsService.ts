...
export const MaxRepsService = {
   ...
   */
  async getMaxReps(userId: string, exerciseId: string, weight?: number): Promise<number | null> {
   ...
   */
  async getAllWeightPRs(userId: string, exerciseId: string): Promise<Map<number | null, number>> {
    const logs = await prisma.exerciseLog.findMany({
      where: { userId, exerciseId },
      select: { sets: true },
    });
