...
export class CombatEngine {
    ...
    static calculatePlayerDamage(attributes: TitanAttributes, actionType: CombatAction['type']): { damage: number; isCrit: boolean } {
    ...
    static calculateBossDamage(boss: Monster, playerAttributes: TitanAttributes, isDefending: boolean): { damage: number; isCrit: boolean } {
    ...
    static processTurn(state: CombatState, action: CombatAction, playerAttributes: TitanAttributes, boss: Monster): TurnResult {
        const events: string[] = [];
        let playerDamage = 0;
        let playerHeal = 0;
        let bossDamage = 0;
        ...
        if (action.type === 'HEAL') {
            ...
            events.push(`You used ${action.type}! Dealt ${damage} damage.${isCrit ? ' CRITICAL HIT!' : ''}`);
        }
        ...
        if (state.bossHp <= 0) {
            ...
            return {
                playerDamage, bossDamage: 0, playerHeal, bossHeal: 0, events, newState: { ...state }
            };
        }
