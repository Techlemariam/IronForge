...
describe("TrainingMemoryManager", () => {
      ...
      const activities: TrainingActivity[] = [
        {
          ...
          intensity: "HIGH",
          currentVolume: 10,
          targets: { mrv: 20, mev: 5, mav: 15, mv: 5 },
          resourceCost: { CNS: 10, MUSCULAR: 10, METABOLIC: 5 },
        },
        {
          name: "Recovery Ride",
          type: "CARDIO_ZONE2",
          intensity: "LOW",
          currentVolume: 30,
          targets: { mrv: 60, mev: 10, mav: 40, mv: 10 },
          resourceCost: { CNS: 2, MUSCULAR: 2, METABOLIC: 5 },
        },
      ];
  ...
  describe("evaluateTransition", () => {
    ...
    it("should trigger GAMMA transition on critically low TSB", () => {
      const metrics = {
        ...baseMetrics,
        tsb: MACRO_CYCLE_THRESHOLDS.GAMMA_TSB_THRESHOLD - 5,
      };
  ...
  describe("shouldEnterSurvivalMode", () => {
    ...
    it("should return false for healthy metrics", () => {
      expect(
        TrainingMemoryManager.shouldEnterSurvivalMode(healthyMetrics),
      ).toBe(false);
    });
    ...
    it("should return true if multiple debuffs are active (Poor Sleep + Low HRV)", () => {
      ...
      expect(TrainingMemoryManager.shouldEnterSurvivalMode(sickMetrics)).toBe(
        true,
      );
    });
    ...
    it("should return true if TSB is critical", () => {
      ...
      expect(TrainingMemoryManager.shouldEnterSurvivalMode(tiredMetrics)).toBe(
        true,
      );
    });
  });
  ...
  describe("canAllocateHighIntensity", () => {
    const lightActivity: TrainingActivity = {
      ...
      intensity: "LOW",
      currentVolume: 10,
      targets: { mrv: 100, mev: 0, mav: 0, mv: 0 },
    ...
    it("should allow high intensity when load is low and capacity is full", () => {
      ...
      expect(
        TrainingMemoryManager.canAllocateHighIntensity([lightActivity], []),
      ).toBe(true);
    });
    ...
    it("should deny high intensity when capacity is heavily debuffed", () => {
      ...
      const debuffs = [
        {
          multiplier: 0.5,
          reason: "Severe Sleep Deprivation",
          source: "SLEEP" as const,
        },
      ];

      expect(
        TrainingMemoryManager.canAllocateHighIntensity(
          [heavyActivity],
          debuffs,
        ),
      ).toBe(false);
