import { getActivitiesAction, getWellnessAction } from '@/actions/integrations/intervals';
import { PATH_VOLUME_MODIFIERS } from '@/data/builds';
import { EXERCISE_DB } from '@/data/exerciseDb';
import prisma from '@/lib/prisma';
import { AutoSpecEngine } from '@/services/game/AutoSpecEngine';
import type { Prisma } from '@/types/prisma';
import type { MacroCycle, NutritionMode, SystemMetrics, TrainingPath } from '@/types/training';
...
export class TrainingContextService {
  ...
  ): Promise<Record<string, VolumeStatus>> {
  ...
  static async getTrainingContext(userId: string): Promise<TrainingContext> {
    ...
    const metrics: SystemMetrics = {
      ...
      hrv: wellness?.hrv || 0,
      sleepScore:
        wellness?.sleepScore || (wellness?.sleepSecs ? (wellness.sleepSecs / 3600 / 8) * 100 : 0),
    ...
    const globalBioMod = nutritionMod * sleepMod * acwrMod;
  ...
  static estimateCnsCost(
    ...
    let baseCost = 3;
    const entry =
      EXERCISE_DB[exerciseName] || EXERCISE_DB[exerciseName.replace(/\s\(.*?\)/, '')] || null;
