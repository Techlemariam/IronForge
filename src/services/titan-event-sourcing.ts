...
export async function replayTitanEvents(
    ...
    const state: Record<string, unknown> = {
        level: 1,
        xp: 0,
        stats: { strength: 10, vitality: 10, endurance: 10, agility: 10, willpower: 10 },
 ...
 */
export async function detectAnomalies(userId: string): Promise<{ suspicious: boolean; reasons: string[] }> {
    ...
    const goldEvents = recentEvents.filter(e => e.eventType === 'GOLD_EARNED' || e.eventType === 'GOLD_SPENT');
