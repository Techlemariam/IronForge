...
export const StorageService = {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('auditor_reports')) {
                    const auditorStore = db.createObjectStore('auditor_reports', { keyPath: 'id', autoIncrement: true });
                    auditorStore.createIndex('timestamp', 'timestamp', { unique: false });
                }

                // NEW (v4): Grimoire Store
                if (!db.objectStoreNames.contains('grimoire')) {
                    const grimoireStore = db.createObjectStore('grimoire', { keyPath: 'id' });
    ...
    async syncToServer(endpoint: string, action: string, payload: any) {
        try {
    ...
    // --- EQUIPMENT SETTINGS ---
    async saveOwnedEquipment(equipment: any[]): Promise<void> {
        return this.saveState('equipment', equipment);
    },

    async getOwnedEquipment(): Promise<any[] | null> {
