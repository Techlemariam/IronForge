...
export const PhysicalSkillService = {
  ...
  async evaluateAndUnlockSkills(userId: string) {
    ...
    if (wilksScore > 250 || bestDeadlift / user.bodyWeight >= 1.5) {
      const unlocked = await this.unlockSkillSafely(
        userId,
        'SKILL_JUGGERNAUT_CLEAVE',
        'Tier 1: Cleave Strike'
      );
    ...
    if (wilksScore > 350 || bestDeadlift / user.bodyWeight >= 2.0) {
      const unlocked = await this.unlockSkillSafely(
        userId,
        'SKILL_JUGGERNAUT_EARTHQUAKE',
        'Tier 3: Earthquake Smash'
      );
    ...
    if (longRuns >= 5) {
      const unlocked = await this.unlockSkillSafely(
        userId,
        'SKILL_PATHFINDER_SWIFTWIND',
        'Tier 1: Swiftwind Dash'
      );
