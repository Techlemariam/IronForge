 ...
 * Replaces the legacy titan-sync-service.ts standalone functions.
 */
export namespace TitanSync {
  ...
  export async function pullTitanState(
    ...
    try {
      ...
      console.error('[TitanSync] Pull sync failed:', error);
      return {
        success: false,
        state: null,
        version: 0,
  ...
  export async function pushTitanState(request: SyncRequest): Promise<SyncResult> {
    ...
    try {
      ...
      console.log(
        `[TitanSync] Sync complete for ${userId} from device ${deviceId}, version ${newVersion}`
      );
      ...
      console.error('[TitanSync] Push sync failed:', error);
      return {
        success: false,
        state: null,
        version: 0,
