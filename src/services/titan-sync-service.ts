...
export async function pushTitanState(request: SyncRequest): Promise<SyncResult> {
    const { userId, deviceId, clientVersion, clientState, lastSyncedAt } = request;
 ...
 */
function detectConflicts(serverState: TitanState | null, clientState: Partial<TitanState>): SyncConflict[] {
    ...
    if (changes.resources) {
        await prisma.titan.update({
            ...
            data: {
                ...
                energy: changes.resources.energy,
            },
        });
    }
