...
import { recordTitanEvent, replayTitanEvents, getTitanEventHistory } from './titan-event-sourcing';
...
export async function createRecoveryCheckpoint(
    ...
    await recordTitanEvent(userId, 'STATE_RECOVERED', { action: 'checkpoint_created', reason });
...
export async function recoverFromCorruption(userId: string): Promise<RecoveryResult> {
    ...
    if (available.length === 0) {
        ...
        try {
            ...
            await recordTitanEvent(userId, 'STATE_RECOVERED', { action: 'reset_to_defaults' });
            revalidatePath('/dashboard');
...
export async function autoHealIntegrityIssues(userId: string): Promise<RecoveryResult> {
    ...
    await recordTitanEvent(userId, 'STATE_RECOVERED', { action: 'auto_heal', issues });
    revalidatePath('/dashboard');
