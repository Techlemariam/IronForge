...
`;
   ...
   *
   * @param powerRating Oracle 3.0 Power Rating from the Titan record. Drives tier-specific decree language.
   */
  static async consult(
    ...
    // --- Oracle 3.0: Determine Tier from Power Rating ---
    // Tiers mirror PowerRatingService.getTierDetails:
    //   Diamond ≥1000 | Platinum ≥750 | Gold ≥500 | Silver ≥250 | Bronze <250
    const getTier = (pr: number) => {
      if (pr >= 1000) return { name: 'Cyber-Titan', label: 'Diamond' };
      if (pr >= 750) return { name: 'Iron Phantom', label: 'Platinum' };
      if (pr >= 500) return { name: 'Forge Master', label: 'Gold' };
      if (pr >= 250) return { name: 'Warrior', label: 'Silver' };
      if (pr > 0) return { name: 'Recruit', label: 'Bronze' };
      return { name: 'Initiate', label: 'Unranked' };
    };
    const tier = getTier(powerRating);

    const recommendation: OracleRecommendation = {
      type: 'GRIND', // Default type
      title: 'Daily Training',
      rationale: `[${tier.label}] ${tier.name} — maintain your grind.`,
    ...
    if (_auditReport?.mobility && _auditReport.mobility.neglectedRegions.length > 0) {
      ...
      if (recommended) {
        ...
        recommendation.rationale = `[${tier.label}] Your ${topNeglected} mobility is lagging. Try ${recommended.exerciseName}. ${_auditReport.mobility.insight}`;
    ...
    if (activePath === 'PATHFINDER') {
      ...
      recommendation.rationale = `[${tier.label}] ${tier.name}: Focus on cardiovascular efficiency.`;
      ...
      recommendation.rationale = `[${tier.label}] ${tier.name}: Focus on heavy compound lifts.`;
    ...
    if (recovery < 30 || (recoveryAnalysis && recoveryAnalysis.state === 'RECOVERY')) {
      ...
      recommendation.rationale = `[${tier.label}] ${tier.name}: System fatigue detected. Prioritize mobility and light movement.`;
    ...
    // Oracle 3.0: Elite Tier boost — Cyber-Titan / Forge Master PR window
    if (powerRating >= 500 && recovery >= 70) {
      recommendation.type = 'PR_ATTEMPT';
      recommendation.title = '⚡ PR Window Open';
      recommendation.rationale = `[${tier.label}] ${tier.name}: Power Rating ${powerRating.toFixed(0)} + high battery (${recovery}%). Conditions are optimal for a Personal Record attempt.`;
      recommendation.priorityScore = 88;
    }

    // Oracle 3.0: Initiate nudge — no Power Rating yet
    if (powerRating === 0) {
      recommendation.rationale =
        'Complete your first training session to unlock your Power Rating and get personalised decrees.';
    }

    // Titan Decree Override (highest priority)
    if (titanState?.dailyDecree?.type === 'DEBUFF') {
      recommendation.type = 'TAPER';
      recommendation.title = 'Tactical Retreat';
      recommendation.rationale = `[${tier.label}] ${tier.name}: Titan Decree (${titanState.dailyDecree.label}) dictates caution.`;
