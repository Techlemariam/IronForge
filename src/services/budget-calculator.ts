...
import type { DailyResourceBudget, SystemMetrics } from '@/types/goals';
...
export namespace BudgetCalculator {
  /**
  ...
  export function wellnessToSystemMetrics(
    ...
    return {
      ...
      atl: wellness?.atl ?? 0,
      ctl: wellness?.ctl ?? 0,
      acwr: wellness?.atl && wellness?.ctl && wellness.ctl > 0 ? wellness.atl / wellness.ctl : 1.0,
      sleepScore: wellness?.sleepScore ?? 80,
      bodyBattery: wellness?.bodyBattery ?? 50,
      soreness: wellness?.soreness ?? 0,
      mood:
        wellness?.mood !== null && wellness?.mood !== undefined
          ? wellness.mood < -3
            ? 'EXHAUSTED'
            : wellness.mood < 0
              ? 'TIRED'
