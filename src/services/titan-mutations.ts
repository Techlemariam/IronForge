 ...
 * Modify Titan stats.
 */
export async function mutateTitanStats(input: z.infer<typeof ModifyStatsSchema>): Promise<MutationResult> {
 ...
 */
export async function mutateTitanResources(input: z.infer<typeof ModifyResourcesSchema>): Promise<MutationResult> {
    ...
    try {
        ...
        if (changes.energy !== undefined) {
            updates.energy = Math.max(0, Math.min(100, titan.energy + changes.energy));
        }
 ...
 */
export async function mutateTitanEconomy(input: z.infer<typeof ModifyEconomySchema>): Promise<MutationResult> {
...
export async function mutateTitanBatch(
    ...
    for (const mutation of mutations) {
        ...
        switch (mutation.type) {
            case 'xp':
                result = await mutateTitanXp({ userId, ...(mutation.payload as { amount: number; source: 'WORKOUT' }) });
                break;
            case 'stats':
                result = await mutateTitanStats({ userId, ...(mutation.payload as { changes: Record<string, number>; source: string }) });
                break;
            case 'resources':
                result = await mutateTitanResources({ userId, ...(mutation.payload as { changes: Record<string, number>; source: string }) });
                break;
            case 'economy':
                result = await mutateTitanEconomy({ userId, ...(mutation.payload as { changes: Record<string, number>; source: string }) });
                break;
            default:
