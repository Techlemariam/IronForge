...
interface OracleRecommendation {
  verdict: 'TRAIN_HARD' | 'TRAIN_NORMAL' | 'TRAIN_LIGHT' | 'REST' | 'ACTIVE_RECOVERY';
...
async function buildOracleContext(userId: string): Promise<OracleContext> {
  ...
  const recentLogs = user?.exerciseLogs.filter((l) => new Date(l.date) >= weekAgo) || [];
...
export async function getOracleRecommendationAction(
  ...
  try {
    ...
    if (wellnessData?.sleepScore !== undefined && wellnessData.sleepScore < 60) {
...
export async function getOracleWeeklySummaryAction(userId: string): Promise<{
  ...
  return {
    weekScore,
    achievements: achievements.length > 0 ? achievements : ['Showing up consistently'],
    areasToImprove: areasToImprove.length > 0 ? areasToImprove : ['Keep up the good work!'],
    nextWeekFocus:
