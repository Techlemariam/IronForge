...
import { validateHevyApiKey, connectHevy, disconnectHevy, validateIntervalsCredentials, connectIntervals, disconnectIntervals } from '../integrations';
...
describe('Integration Actions', () => {
    ...
    describe('Hevy Integrations', () => {
        it('validateHevyApiKey should return valid true on success', async () => {
            vi.spyOn(libHevy, 'getHevyWorkouts').mockResolvedValue({ workouts: [], page_count: 0 });
            const result = await validateHevyApiKey(apiKey);
            expect(result).toEqual({ valid: true });
        });

        it('validateHevyApiKey should return error on failure', async () => {
            vi.spyOn(libHevy, 'getHevyWorkouts').mockRejectedValue(new Error('Invalid key'));
            const result = await validateHevyApiKey(apiKey);
    ...
    describe('Intervals.icu Integrations', () => {
        ...
        it('validateIntervalsCredentials should return valid on success', async () => {
            vi.spyOn(libIntervals, 'getAthleteSettings').mockResolvedValue({ name: 'Test Athlete' } as any);
            const result = await validateIntervalsCredentials(apiKey, athleteId);
            expect(result).toEqual({ valid: true, metadata: { name: 'Test Athlete' } });
        });
