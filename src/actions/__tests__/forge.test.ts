...
describe('Forge Server Actions', () => {
    ...
    it('should successfully craft an item if resources are sufficient', async () => {
        mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user-1', email: 'test@example.com' } } });
        ...
        const ore = result.inventory?.items.find(i => i.itemId === 'item_iron_ore');
        expect(ore?.count).toBe(8); // 10 - 2

        // Verify Addition
        const sword = result.inventory?.items.find(i => i.itemId === 'item_iron_sword');
