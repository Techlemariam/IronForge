...
import { sendChatAction, attackBossAction, getUserStatsAction } from '../guild';
...
describe('Guild Actions', () => {
    ...
    describe('attackBossAction', () => {
        ...
        it('should throw if insufficient energy', async () => {
            (prisma.user.findUnique as any).mockResolvedValue({ ...mockUser, kineticEnergy: 0 });
            (prisma.raidBoss.findUnique as any).mockResolvedValue({
                id: 'boss-1',
                isActive: true,
                currentHp: BigInt(1000),
            ...
            await expect(attackBossAction('boss-1')).rejects.toThrow("Insufficient Kinetic Energy");
        });
    });
