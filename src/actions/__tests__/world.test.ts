...
import { getWorldStateAction, getBestiaryAction } from '../world';
...
describe('World Server Actions', () => {
    ...
    describe('getWorldStateAction', () => {
        ...
        it('should return regions with correct unlocked state', async () => {
            mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user-1' } } });
            (prisma.user.findUnique as any).mockResolvedValue({ level: 5, heroName: 'Hero' });
        ...
        it('should obscure details for regions far above user level', async () => {
            ...
            (prisma.user.findUnique as any).mockResolvedValue({ level: 1, heroName: 'Novice' });

            (prisma.worldRegion.findMany as any).mockResolvedValue([
                { id: 'region-void', name: 'The Void', description: 'Scary', levelReq: 20, coordX: 100, coordY: 100 },
            ]);
