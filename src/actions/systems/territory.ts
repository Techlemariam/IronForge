...
import {
  contestTerritoryAction as contestTerritoryInternal,
  getContestLeaderboardAction as getContestLeaderboardInternal,
...
export async function getTerritoryAppData() {
  ...
  const mapTiles: MapTile[] = controlRecords.map((c) => {
    ...
    if (user?.homeLatitude && user?.homeLongitude) {
      if (
        isWithinHomeZone(c.tileId, user.homeLatitude, user.homeLongitude, HOME_ZONE_RADIUS_METERS)
      ) {
        state = 'HOME_ZONE';
      }
    }
