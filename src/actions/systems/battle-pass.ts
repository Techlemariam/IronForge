 ...
 */
async function getActiveSeasonInternal() {
 ...
 * Get the currently active Battle Pass season.
 */
export const getActiveSeasonAction = actionClient.action(async () => {
  try {
    return await getActiveSeasonInternal();
  } catch (error) {
 ...
 */
export const getUserBattlePassProgressAction = authActionClient.action(
  async ({ ctx: { userId } }): Promise<BattlePassProgress | null> => {
    try {
      const season = await getActiveSeasonInternal();
 ...
 */
const AddBattlePassXpSchema = z.object({ amount: z.number().int() });
export const addBattlePassXpAction = authActionClient
  .schema(AddBattlePassXpSchema)
...
export const claimBattlePassRewardAction = authActionClient
  .schema(ClaimRewardSchema)
  .action(async ({ parsedInput: { tierLevel, isPremium }, ctx: { userId } }) => {
    try {
      const season = await getActiveSeasonInternal();
      if (!season) return { success: false, message: "No active season" };
 ...
 */
export const upgradeToPremiumAction = authActionClient.action(
  async ({ ctx: { userId } }) => {
    try {
      const season = await getActiveSeasonInternal();
      if (!season) return { success: false, message: "No active season" };
