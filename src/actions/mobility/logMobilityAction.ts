 ...
 */
export async function logMobilitySession(exerciseId: string, durationSecs: number, notes?: string) {
  try {
    ...
    const difficultyMultiplier =
      exerciseData.difficulty === 'ADVANCED'
        ? 1.5
    ...
    const weeklyMinutes =
      weeklyLogs.reduce((sum: number, log) => sum + log.durationSecs / 60, 0) + durationSecs / 60;
