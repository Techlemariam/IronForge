...
export async function getMaxRepsAction(
  ...
  try {
    ...
    const maxReps = await MaxRepsService.getMaxReps(session.user.id, exerciseId, weight);
...
export async function checkPRAction(
  ...
  try {
    ...
    const previousMax = await MaxRepsService.getMaxReps(session.user.id, exerciseId, weight);

    const isPR = await MaxRepsService.checkForPR(session.user.id, exerciseId, reps, weight);
 ...
 */
export async function getPRHistoryAction(exerciseId: string, weight?: number) {
  try {
    const session = await getSession();
    if (!session?.user?.id) {
      return { success: false, error: 'Not authenticated' };
    ...
    const history = await MaxRepsService.getPRHistory(session.user.id, exerciseId, weight);
