...
export async function extractTrainingDnaAction(
  ...
  try {
    ...
    return {
      ...
      splitType: daysPerWeek <= 3 ? 'FULL_BODY' : daysPerWeek <= 4 ? 'UPPER_LOWER' : 'PPL',
      avgRpe: 7.5, // Default
 ...
 */
export async function getPopularDnasAction(_limit = 20): Promise<TrainingDNA[]> {
  // MVP: Return sample DNAs
  return [
    {
      id: 'dna-sample-1',
      name: 'Natural Hypertrophy Blueprint',
      description: 'Moderate volume, compound-focused plan for natural lifters.',
