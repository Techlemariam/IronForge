...
const MUSCLE_SET_RANGES: Record<string, { min: number; optimal: number; max: number }> = {
 ...
 */
export async function getMuscleHeatMapAction(userId: string): Promise<FullBodyHeatMap> {
  try {
    ...
    const muscles: MuscleHeatData[] = Object.entries(MUSCLE_SET_RANGES).map(([muscle, range]) => {
      // Simulate sets based on exercise patterns
      const setsThisWeek = Math.floor(Math.random() * 20);
      const volumeThisWeek = setsThisWeek * 500 + Math.floor(Math.random() * 1000);
      ...
      else if (intensity === 'LOW') recommendedAction = `Increase ${muscle} volume`;
      else if (intensity === 'VERY_HIGH') recommendedAction = `Consider reducing ${muscle} volume`;
      ...
      return {
        ...
        volumeThisWeek,
        lastTrained:
          setsThisWeek > 0 ? new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000) : null,
        intensity,
    ...
    const intensityScores = muscles.map((m) => {
      ...
      if (!range) return 50;
      if (m.setsThisWeek >= range.min && m.setsThisWeek <= range.max) return 100;
    ...
    const sortedByVolume = [...muscles].sort((a, b) => b.setsThisWeek - a.setsThisWeek);
    const mostTrained = sortedByVolume[0]?.muscleGroup || 'None';
    const leastTrained = sortedByVolume[sortedByVolume.length - 1]?.muscleGroup || 'None';

    const recommendations: string[] = [];
    if (overallBalance < 60)
      recommendations.push('Training is imbalanced - consider a more structured program');
    const neglected = muscles.filter((m) => m.intensity === 'NONE' || m.intensity === 'LOW');
    if (neglected.length > 0) {
      recommendations.push(`Focus more on: ${neglected.map((m) => m.muscleGroup).join(', ')}`);
    }
