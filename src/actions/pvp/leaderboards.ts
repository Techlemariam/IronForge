...
async function getPvpLeaderboard(seasonId?: string, limit = 20) {
...
async function getStrengthLeaderboard(
  exerciseId: string,
  limit = 20,
  _scope: 'GLOBAL' | 'COUNTRY' | 'CITY' = 'GLOBAL'
) {
  // Note: Reusing logic from segment-leaderboard for now but normalized
  const logs = await prisma.exerciseLog.findMany({
    where: { exerciseId, weight: { gt: 0 } },
    include: {
      user: { select: { id: true, heroName: true, country: true, city: true, faction: true } },
    },
  });
    ...
    for (const exerciseId of exerciseIds) {
      ...
      if (userRank) {
        ...
        const bestE1rm = Math.max(
          ...logsForEx.map((l) => (l.weight || 0) * (1 + (l.reps || 0) / 30))
        );
