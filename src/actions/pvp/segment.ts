...
const CreateSegmentBattleSchema = z.object({ segmentId: z.string(), opponentId: z.string() });
export const createSegmentBattleAction = authActionClient
  .schema(CreateSegmentBattleSchema)
  .action(async ({ parsedInput: { segmentId, opponentId }, ctx: { userId } }) => {
    ...
    console.log(
      `User ${userId} challenged ${opponentId} on segment ${segmentId}`,
    );

    return { success: true, message: "Challenge sent via carrier pigeon!" };
  });

const ResolveSegmentBattleSchema = z.number();
export const resolveSegmentBattleAction = authActionClient
  .schema(ResolveSegmentBattleSchema)
  .action(async ({ parsedInput: uploadId, ctx: { userId } }) => {
    // 1. Get the upload status to find the activity ID
    // Note: We need the token again.
    const dbUser = await prisma.user.findUnique({ where: { id: userId } });
    if (!dbUser?.stravaAccessToken)
      return { success: false, error: "No Strava Token" };
    ...
    try {
      ...
      if (upload.status === "processed" && upload.activity_id) {
        ...
        const activityRes = await axios.get(
          `https://www.strava.com/api/v3/activities/${upload.activity_id}?include_all_efforts=true`,
          {
            headers: { Authorization: `Bearer ${dbUser.stravaAccessToken}` },
          },
        );
        ...
        return {
          success: true,
          segments: segmentEfforts.map((se: any) => ({
            name: se.name,
