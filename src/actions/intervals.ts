...
async function getIntervalsCredentials() {
    ...
    return { apiKey: dbUser.intervalsApiKey, athleteId: dbUser.intervalsAthleteId };
}
...
export async function getActivitiesAction(startDate: string, endDate: string): Promise<IntervalsActivity[]> {
    try {
        const { apiKey, athleteId } = await getIntervalsCredentials();
...
export async function getEventsAction(startDate: string, endDate: string): Promise<IntervalsEvent[]> {
    try {
        const { apiKey, athleteId } = await getIntervalsCredentials();
