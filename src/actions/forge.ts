...
export async function craftItem(recipeId: string): Promise<{ success: boolean; message: string; inventory?: UserInventory }> {
    try {
        ...
        if (resultSlot) {
            ...
            inventory.items.push({ itemId: recipe.resultItemId, count: recipe.resultCount });
        }
        ...
        return { success: true, message: `Successfully crafted ${recipe.name}`, inventory };
