...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!event.payload || !event.payload.workoutId) {
            return NextResponse.json({ error: 'Invalid payload: workoutId missing' }, { status: 400 });
        }

        const workoutId = event.payload.workoutId;
        console.log(`[Hevy Webhook] Notification received for Workout ID: ${workoutId}`);
        ...
        console.log(`[Hevy Webhook] Fetched full workout: ${workoutData.title} (${workoutData.exercises?.length} exercises)`);
