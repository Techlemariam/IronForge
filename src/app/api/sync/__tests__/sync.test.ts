...
describe('Sync API Route (Logs)', () => {
    ...
    it('should save exercise log correctly', async () => {
        ...
        expect(LogService.saveExerciseLog).toHaveBeenCalledWith('user-1', { exercise: 'Squat', weight: 100 });
    });
    ...
    it('should save meditation log correctly', async () => {
        ...
        expect(data.success).toBe(true);
        expect(LogService.saveMeditationLog).toHaveBeenCalledWith('user-1', { duration: 10 });
    });
