...
export default async function GrowthDashboardPage() {
  ...
  const userProgress = Math.min(
    100,
    (businessTriggers.recurringUsers.current / businessTriggers.recurringUsers.target) * 100
  );
  const retentionMet =
    businessTriggers.retentionRate.current >= businessTriggers.retentionRate.target;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm">
              Path to Passive Income • Updated {new Date(snapshot.timestamp).toLocaleString()}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded ${userProgress >= 100 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}
                >
              ...
              <div className="text-4xl font-black text-white">
                ...
                <span className="text-lg text-zinc-500">
                  /{businessTriggers.recurringUsers.target}
                </span>
              </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded ${retentionMet ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}
                >
              ...
              <div className="text-xs text-zinc-600 mt-2">
                Target: ≥{businessTriggers.retentionRate.target}%
              </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-bold px-2 py-1 rounded bg-zinc-800 text-zinc-400">
                  MANUAL
                </span>
              </div>
              ...
              <div className="text-xs text-zinc-600 mt-2">
                Check Discord/Support for &quot;Can I pay?&quot;
              </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              ...
              <div className="text-xs text-zinc-600 mt-2">
                Target: &gt;500 SEK/mo (Coolify/Supabase)
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <div className="text-3xl font-black text-white">
                {funnel.onboardingCompletionRate}%
              </div>
              <div className="text-sm text-zinc-500">Onboarding Completion</div>
              <div
                className={`text-xs mt-2 ${funnel.onboardingCompletionRate >= 50 ? 'text-emerald-400' : 'text-red-400'}`}
              >
