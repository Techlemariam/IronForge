...
export function TitanXPBar({ currentXP, maxXP, level = 1, isRested = false, isElite = false }: TitanXPBarProps) {
    ...
    <div className="w-full font-serif relative">
      ...
      {showLevelUp && (
          <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md animate-fade-in-fast pointer-events-none">
             ...
             <div className="flex items-baseline gap-4 text-white animate-fade-in-delayed">
                <span className="text-2xl font-sans uppercase tracking-widest text-zinc-400">Titan Rank</span>
      ...
      <div className="flex justify-between items-end mb-2 px-1">
        <div className="flex items-baseline gap-2">
          <span className={`text-xs font-bold tracking-widest uppercase ${isElite ? 'text-[#ffd700]' : 'text-zinc-500'}`}>
            Level
          </span>
          <span className={`text-3xl font-black drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] ${isElite ? 'text-white text-shadow-gold' : 'text-white'}`}>
            {level}
          </span>
        </div>
        
        <div className="text-right">
          <span className={`font-mono font-bold text-sm ${isElite ? 'text-white' : isRested ? "text-blue-400" : "text-[#ffd700]"}`}>
            {animatedXP} 
      ...
      {/* The Bar Container */}
      <div className={`relative h-6 bg-zinc-950 rounded-full border ${containerBorder} overflow-hidden transition-all duration-500`}>
        ...
        >
          ...
          <div className={`absolute inset-0 bg-white z-20 transition-opacity duration-300 ${flash ? 'opacity-80' : 'opacity-0'}`} />
        ...
        <div className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none">
          <span className={`text-[9px] font-black uppercase tracking-[0.2em] drop-shadow-sm mix-blend-overlay ${isElite ? 'text-black/60' : 'text-white/40'}`}>
