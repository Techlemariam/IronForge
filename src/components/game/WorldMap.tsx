...
import { Map as MapIcon, Lock, Compass, Mountain, Waves, Flame } from 'lucide-react';
...
export const WorldMap: React.FC<WorldMapProps> = ({ userLevel, onClose }) => {
    ...
    const getZoneIcon = (icon: string) => {
        switch (icon) {
            case '🌋': return <Flame className="w-10 h-10" />;
            case '🌪️': return <Mountain className="w-10 h-10" />; // Proxy for height
            case '🌊': return <Waves className="w-10 h-10" />;
                    ...
                    return (
                        ...
                        >
                            <div className="flex flex-col items-center">
                                ...
                                >
                                    {unlocked ? getZoneIcon(zone.icon) : <Lock className="w-8 h-8" />}
