...
import type { TerritoryWithControl } from '@/actions/systems/territories';
      ...
      >
        ...
        >
          <div className="absolute top-2 right-2">
            <Badge
              variant="outline"
              className="bg-black/60 backdrop-blur border-white/10 font-mono text-[9px] uppercase tracking-wider text-zinc-400"
            >
              {territory.region}
            </Badge>
          </div>
        ...
        <div className="pt-6 px-4 pb-4">
          <div className="flex justify-between items-start mb-2">
            <div className="pr-2">
              <h3 className="font-black text-base text-white leading-tight uppercase tracking-wide">
                {territory.name}
              </h3>
          ...
          {isControlled ? (
            <div className="mb-3 p-2 rounded bg-white/5 border border-white/5">
              <div className="text-[9px] text-zinc-500 uppercase tracking-widest mb-0.5">
                Controlled By
              </div>
              <div
                className={cn(
                  'font-bold text-xs truncate',
                  isMyGuild ? 'text-emerald-400' : 'text-red-400'
                )}
              >
          ...
          <div className="flex gap-1.5 flex-wrap">
            {territory.bonuses.xpBonus && (
              <Badge
                variant="secondary"
                className="bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 text-[10px] h-5 px-1.5 border-0 rounded-sm"
              >
            ...
            {territory.bonuses.goldBonus && (
              <Badge
                variant="secondary"
                className="bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 text-[10px] h-5 px-1.5 border-0 rounded-sm"
              >
            ...
            {territory.bonuses.defenseBonus && (
              <Badge
                variant="secondary"
                className="bg-slate-500/10 text-slate-400 hover:bg-slate-500/20 text-[10px] h-5 px-1.5 border-0 rounded-sm"
              >
          ...
          <div className="mt-3 pt-3 border-t border-white/5 flex justify-between items-center">
            <div className="text-[10px] text-zinc-500 font-mono">
              {territory.contestProgress ? (
                <span className="text-zinc-300">
                  {territory.contestProgress.entries.length} Guilds
                </span>
              ) : (
            ...
            <Button
              size="sm"
              variant="ghost"
              className="h-6 text-[10px] uppercase font-bold text-zinc-400 hover:text-white hover:bg-white/5 px-2"
            >
              Scout
            </Button>
          </div>
