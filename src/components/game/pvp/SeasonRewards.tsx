...
import { type Faction, PVP_RANKS, getPointsToNextRank, getRankProgress } from '@/lib/pvpRanks';
...
export function SeasonRewards({
  ...
  // Find current rank
  const currentRankData = PVP_RANKS.reduce(
    (prev, curr) => (currentRating >= curr.minRankScore ? curr : prev),
    PVP_RANKS[0]
  );
  ...
  const daysLeft = Math.ceil(
    (new Date(seasonEndDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
  );
  ...
  return (
    <Card className="p-4 bg-zinc-900 border-zinc-800 shadow-xl">
      ...
      <div className="text-xs text-center p-3 bg-zinc-950/50 rounded border border-zinc-800/50 flex justify-between items-center px-4">
        <span className="text-zinc-500 uppercase font-bold text-[10px] tracking-wider">
          {title} Reward
        </span>
        <span className="text-yellow-400 font-bold font-mono text-sm shadow-yellow-500/20 drop-shadow-sm">
          {currentReward}
        </span>
      </div>
    </Card>
