...
const BattleGearSection: React.FC = () => {
    ...
    return (
        <div className="animate-fade-in mt-12 pt-12 border-t border-zinc-800">
            <h2 className="text-xl font-bold text-yellow-500 uppercase tracking-widest mb-6">Battle Gear (Active Effects)</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {inventory.map((itemId, idx) => (
                    <div key={idx} className="bg-zinc-900 border border-yellow-900/50 p-4 rounded flex items-center gap-4">
                        <div className="p-2 bg-yellow-900/20 rounded border border-yellow-700/50 text-yellow-500 text-xs font-mono">
                            ITEM
                        </div>
                        <div>
                            <div className="text-white font-bold uppercase">{itemId.replace('scroll_', 'Scroll of ').replace('potion_', 'Potion of ').replace('shield_', 'Shield of ').replace('_', ' ')}</div>
...
const EquipmentArmory: React.FC = () => {
    ...
    const toggleEquipment = async (type: EquipmentType) => {
        ...
        setOwned(newOwned);
        await StorageService.saveOwnedEquipment(newOwned);
    };
    ...
    if (loading) return <div className="p-8 text-center text-forge-muted">Accessing Armory Database...</div>;

    return (
        <div className="w-full h-full p-4 md:p-8 overflow-y-auto animate-fade-in">
            <div className="max-w-4xl mx-auto space-y-8">
                <header className="mb-8">
                    <h1 className="text-3xl font-heading tracking-widest text-white mb-2">ARMORY</h1>
                    <p className="font-mono text-forge-muted">Configure your available arsenal. The Oracle will adapt.</p>
                </header>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            ...
                            <p className="text-sm text-forge-muted mt-1 max-w-xl">
                                Enable priority scheduling for <span className="text-white">Freak Athlete Hyper Pro</span>.
                                The Oracle will aggressively suggest 10-in-1 compatible variations (Nordics, GHDs, Reverse Hypers) whenever possible.
                            </p>
                        </div>
                        <div className="flex items-center gap-3 bg-forge-900/50 p-3 rounded-lg border border-forge-border">
                            ...
                            />
                            <span className="font-mono text-sm font-bold uppercase">{hyperProMode ? 'ACTIVE' : 'OFFLINE'}</span>
                        </div>
                    </div>
                </ForgeCard>
                        ...
                        return (
                            <div
                                ...
                                className={`
                                    ...
                                    ${isSelected
                                        ? isHyperPro ? 'bg-rarity-epic/10 border-rarity-epic' : 'bg-forge-800 border-rarity-common'
                                        : 'bg-forge-900/30 border-forge-border hover:border-gray-500'
                                    }
                                `}
                            >
                                <span className={`font-heading tracking-wider ${isSelected ? 'text-white' : 'text-gray-500'}`}>
                                    {eq.label}
                                </span>
                                <div className={`w-4 h-4 rounded-sm border ${isSelected ? (isHyperPro ? 'bg-rarity-epic border-rarity-epic' : 'bg-rarity-common border-rarity-common') : 'border-gray-600'}`}></div>
                            </div>
                        );
                    })}
                </div>
