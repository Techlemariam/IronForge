...
export function PowerRatingBadge({ rating, className }: PowerRatingBadgeProps) {
  ...
  const getTierColor = (r: number) => {
    ...
    if (r >= 750) return 'text-rarity-gold drop-shadow-[0_0_8px_var(--color-gold)]'; // Gold
  ...
  return (
    <div className={cn('flex flex-col items-center', className)}>
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={cn('text-2xl font-black font-mono tracking-tighter', getTierColor(rating))}
        >
          {rating}
        </span>
      </div>
