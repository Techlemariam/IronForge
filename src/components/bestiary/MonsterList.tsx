...
export function MonsterList({ initialMonsters }: { initialMonsters: BestiaryMonster[] }) {
    const [search, setSearch] = useState('')
    // const [selectedId, setSelectedId] = useState<string | null>(null) // Unused for now, but good to keep if we add detailed modal

    const filteredMonsters = initialMonsters.filter(m =>
                        ...
                        <div className="p-6">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-forge-muted uppercase tracking-wider">{monster.title}</p>
                                </div>
                                <span className={`text-[10px] font-mono uppercase px-2 py-1 rounded border ${DIFFICULTY_COLORS[monster.difficulty]}`}>
                                    {monster.difficulty}
                                </span>
                            </div>

                            <div className="flex gap-4 mb-4 text-xs font-mono text-gray-400">
                                <span className="flex items-center gap-1"><Shield className="w-3 h-3" /> {monster.type}</span>
                                <span className="flex items-center gap-1"><Sword className="w-3 h-3" /> Weakness: {monster.weakness}</span>
                            </div>
