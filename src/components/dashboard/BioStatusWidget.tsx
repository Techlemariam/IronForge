  ...
  return (
    <div className="mechanical-panel p-5 space-y-5 group overflow-hidden">
      ...
      <div className="relative flex items-center justify-between z-10">
        ...
        <div
          className={cn(
            'text-[10px] font-mono px-2 py-0.5 border bg-black/40 tracking-tighter',
            readinessColor
          )}
        >
          {readiness.replace('_', ' ')}
      ...
      <div className="relative space-y-2 z-10">
        <div className="flex justify-between items-end">
          ...
          <span className="text-[10px] font-mono text-slate-300">
            {globalLoad} <span className="text-slate-600">/</span> {globalMrv}{' '}
        ...
        >
          ...
          <div
            className={cn(
              'absolute top-0 left-0 h-full transition-all duration-1000 ease-out border-r border-white/20',
              capacityPct > 100
                ? 'bg-red-950/60'
                : capacityPct > 80
                  ? 'bg-amber-500/40'
          ...
          {globalMrv < 150 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <span className="text-[8px] font-mono text-orange-500/50 animate-pulse tracking-[0.2em]">
                CONSTRAINT ACTIVE
              </span>
            </div>
          )}
