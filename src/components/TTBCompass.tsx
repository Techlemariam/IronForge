...
const TTBCompass: React.FC<TTBCompassProps> = ({ indices }) => {
  ...
  const healthStatus = healthPercentage > 85 ? 'FRESH' : healthPercentage > 60 ? 'STABLE' : healthPercentage > 30 ? 'FATIGUED' : 'DEPLETED';
  const healthColor = healthPercentage > 70 ? 'bg-rarity-rare' : healthPercentage > 40 ? 'bg-warrior-light' : 'bg-magma';
            ...
            <div className="relative flex flex-col items-center">
                <div className="relative w-40 h-40">
                    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full drop-shadow-2xl overflow-visible">
                        ...
                        <circle cx={pEndurance.x} cy={pEndurance.y} r="3" fill="#ffd700" />

                        {/* Labels */}
                        <text x={maxWellness.x - 10} y={maxWellness.y + 5} textAnchor="end" fill="#c79c6e" fontSize="8" fontWeight="bold" className="uppercase font-serif">HEL</text>
                        <text x={maxStrength.x + 10} y={maxStrength.y + 5} textAnchor="start" fill="#c79c6e" fontSize="8" fontWeight="bold" className="uppercase font-serif">STR</text>
                        <text x={maxEndurance.x - 10} y={maxEndurance.y + 5} textAnchor="end" fill="#c79c6e" fontSize="8" fontWeight="bold" className="uppercase font-serif">END</text>
                ...
                <div className="w-full px-2 mt-2">
                    <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-serif text-warrior">HEALTH (TSB)</span>
