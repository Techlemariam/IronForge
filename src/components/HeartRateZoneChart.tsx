...
const ZONE_GAMIFICATION = [
    { name: 'Recovery', label: 'Regeneration', color: '#10b981', desc: 'Active recovery & healing' }, // Z1
    { name: 'Endurance', label: 'Endurance Capacitor', color: '#3b82f6', desc: 'Base building & fat burn' }, // Z2
    { name: 'Tempo', label: 'Rhythm Flow', color: '#6366f1', desc: 'Aerobic power' }, // Z3
    { name: 'Threshold', label: 'Redline Limit', color: '#f59e0b', desc: 'Lactate threshold' }, // Z4
    { name: 'VO2 Max', label: 'Nitro Boost', color: '#ef4444', desc: 'Max aerobic capacity' }, // Z5
    { name: 'Anaerobic', label: 'Warp Drive', color: '#ec4899', desc: 'Anaerobic capacity' }, // Z6
    { name: 'Neuromuscular', label: 'Overdrive', color: '#8b5cf6', desc: 'Pure sprint power' }, // Z7
];

export const HeartRateZoneChart: React.FC<HeartRateZoneChartProps> = ({ activities, settings }) => {
    // Aggregation Logic
        ...
        <div className="bg-black/40 backdrop-blur-md border border-white/5 rounded-2xl p-6 shadow-xl">
            ...
            <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={zoneData} layout="vertical" margin={{ left: 0, right: 30 }}>
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
                {zoneData.filter(z => z.value > 0).slice(0, 4).map(zone => (
                    <div key={zone.id} className="bg-white/5 rounded p-2 border border-white/5">
                        ...
                        <div className="text-xs text-zinc-300 font-medium truncate" title={zone.desc}>{zone.desc}</div>
                    </div>
                ))}
            </div>
        </div>
