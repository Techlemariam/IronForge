...
import type { ModelQuota } from '@/services/game/FactoryService';
import {
  AlertTriangle,
  BrainCircuit,
  DollarSign,
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Emergency Protocols */}
        <div
          className={`relative overflow-hidden p-6 rounded-2xl border transition-all duration-500 ${isEmergencyStop ? 'border-red-500 bg-red-500/10' : 'border-slate-800 bg-slate-900/40'}`}
        >
          <div className="flex items-center justify-between mb-4">
            <button
            ...
            <span
              className={`text-[10px] font-bold uppercase tracking-widest ${isEmergencyStop ? 'text-red-400' : 'text-slate-500'}`}
            >
          ...
          <div className="space-y-1">
            <h3
              className={`text-2xl font-black font-mono ${isEmergencyStop ? 'text-red-500' : 'text-white'}`}
            >
