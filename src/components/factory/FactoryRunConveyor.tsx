...
import type { AssemblyLineTask } from '@/services/game/FactoryService';
import { AnimatePresence, motion } from 'framer-motion';
import { Activity, Code2, Cpu, Layout, Rocket, ShieldCheck, Zap } from 'lucide-react';
        ...
        {STAGES.map((stage, idx) => {
          ...
          return (
            <div key={stage.id} className="relative z-10 flex flex-col items-center gap-4 flex-1">
              ...
              {idx > 0 && (
                <div
                  className={`absolute right-1/2 top-8 w-full h-[1px] -z-10 transition-colors duration-700 ${
              ...
              <motion.div
                ...
                className={`relative w-16 h-16 rounded-2xl border bg-slate-900 flex items-center justify-center transition-all duration-500 ${
                  isActive ? `${stage.glow} border-indigo-500/50 ring-4 ring-indigo-500/5` : ''
                } ${isCompleted ? 'border-emerald-500/30' : 'border-slate-800'}`}
              >
                <stage.icon
                  className={`w-7 h-7 transition-colors duration-500 ${
                    isActive ? stage.color : isCompleted ? 'text-emerald-500/50' : 'text-slate-600'
                  }`}
                />
          ...
          {[...Array(5)].map((_, i) => (
            <div
              ...
              className={`w-1 h-3 rounded-full transition-colors duration-700 ${
                activeRun ? 'bg-indigo-500/50' : 'bg-slate-800'
              }`}
