...
import { Cloud, X, Shield, RefreshCw, CheckCircle2, Server, Wifi } from 'lucide-react';
...
const ValhallaGate: React.FC<ValhallaGateProps> = ({ isOpen, onClose, heroName, onBind, syncPayload }) => {
         ...
         <div className="p-8 space-y-6 relative z-10">
             ...
             {step === 'BIND' && (
                 <div className="space-y-4 animate-slide-up">
                     <div className="text-center">
                         ...
                         <h3 className="text-cyan-100 font-bold uppercase tracking-widest mb-2">Bind Your Soul</h3>
                     ...
                     <div className="space-y-2">
                         <label className="text-[10px] font-bold uppercase text-cyan-700 tracking-widest">Hero Name</label>
                         <input 
                            value={inputName}
             ...
             {step === 'IDLE' && (
                 <div className="space-y-6 animate-slide-up">
                     ...
                     <div className="space-y-2">
                        <h4 className="text-[10px] font-bold uppercase text-cyan-800 border-b border-cyan-900/50 pb-1">Pending Upload</h4>
                        <div className="grid grid-cols-2 gap-2 text-xs text-cyan-600 font-mono">
                            <div>XP Level: <span className="text-cyan-300">{finalPayload?.level || 0}</span></div>
                            <div>Achievements: <span className="text-cyan-300">{finalPayload?.achievements.length || 0}</span></div>
                            <div>Talents: <span className="text-cyan-300">{finalPayload?.skills.length || 0}</span></div>
                            <div>Logs: <span className="text-cyan-300">{finalPayload?.historyCount || 0}</span></div>
                        </div>
                     </div>
                     ...
                     >
                         ...
                         <span className="text-[9px] text-cyan-600 font-sans group-hover:text-cyan-400">Overwrites Remote Data</span>
                     </button>
                 </div>
             )}
