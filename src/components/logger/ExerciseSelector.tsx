...
import { createExerciseAction, searchExercisesAction } from '@/actions/user/logger';
...
import { type EquipmentType, canPerformExercise } from '@/data/equipmentDb';
import type { Exercise } from '@/types/prisma';
import { Dumbbell, Lock, Plus, Search } from 'lucide-react';
      ...
      <div className="space-y-2">
        ...
        {!isSearching && results.length > 0 && (
          <div className="grid gap-2">
            {results.map((ex) => {
              const isPerformable = capabilities ? canPerformExercise(ex.name, capabilities) : true;
...
function CreateExerciseDialog({
  initialName,
  onSuccess,
}: { initialName: string; onSuccess: (ex: Exercise) => void }) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="bg-zinc-950 border-zinc-800 text-zinc-200">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={muscle} onValueChange={setMuscle}>
              ...
              <SelectContent className="bg-zinc-900 border-zinc-700">
                {['Chest', 'Back', 'Legs', 'Shoulders', 'Arms', 'Core', 'Cardio', 'Other'].map(
                  (m) => (
          ...
          <Button fullWidth variant="magma" onClick={handleCreate} disabled={isSubmitting}>
