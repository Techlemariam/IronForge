...
import { AlertTriangle, TrendingUp, Sword, HeartPulse, ArrowRight, Skull, Sparkles } from 'lucide-react';
...
const OracleCard: React.FC<OracleCardProps> = ({ recommendation, onAccept }) => {
    ...
    return (
        <div className={`relative p-6 rounded-lg border-2 ${theme.border} ${theme.bg} ${theme.glow} transition-all duration-500 flex flex-col md:flex-row items-start md:items-center gap-6 overflow-hidden`}>
            ...
            <div className="flex-1 z-10 space-y-2">
                <div className="flex items-center gap-2">
                    <span className={`text-[10px] uppercase font-bold tracking-[0.2em] ${theme.accent}`}>The Oracle Commands</span>
            ...
            {(recommendation.sessionId || recommendation.generatedSession) && (
                ...
                >
                    <span>{recommendation.generatedSession ? 'Accept Special Quest' : 'Initiate Protocol'}</span>
