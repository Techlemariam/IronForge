...
const UltrathinkDashboard: React.FC<UltrathinkDashboardProps> = ({ wellness, audit, forecast, ttb, events = [], titanAnalysis, activePath = 'HYBRID_WARDEN' }) => {
    ...
    const displayTitan: TitanLoadCalculation = titanAnalysis || { standardTss: 0, titanLoad: 0, discrepancy: 0, advice: "No recent weight training detected." };
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <div className="flex flex-col">
                                ...
                                <span className="text-xl font-mono font-bold text-zinc-400">{displayTitan.standardTss} TSS</span>
                            </div>
                            ...
                            <div className="flex flex-col text-right">
                                ...
                                <span className="text-xl font-mono font-bold text-orange-500">{displayTitan.titanLoad} TL</span>
                            </div>
                        </div>
                        {titanAnalysis && <div className="text-[10px] text-zinc-500 italic border-t border-zinc-800 pt-2">{titanAnalysis.advice}</div>}
                    </div>
                </div>
            </div>
                        ...
                        return (
                            <div key={day.dayOffset} className="flex items-center gap-4 text-xs">
                                ...
                                <div className="flex-1 h-2 bg-zinc-800 rounded-full overflow-hidden relative">
                                    <div
                                        className={`absolute h - full rounded - full transition - all ${day.tsb < 0 ? 'right-1/2 bg-red-500' : 'left-1/2 bg-green-500'
                                            } `}
                                        style={{
