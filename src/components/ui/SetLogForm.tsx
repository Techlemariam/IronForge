...
const SetLogForm: React.FC<SetLogFormProps> = ({ targetWeight, targetReps, targetRPE, previousWeight, onLog }) => {
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-[#050505] p-2 rounded border border-zinc-800 flex flex-col items-center gap-2">
                ...
                <div className="flex items-center w-full justify-between">
                    <button onClick={() => adjustWeight(-2.5)} className="w-10 h-10 bg-zinc-900 border border-zinc-700 rounded hover:bg-red-900/30 hover:border-red-700 text-zinc-400 active:scale-95 transition-all">
                        <Minus className="w-5 h-5 mx-auto" />
                    </button>
                    <span className="text-3xl font-mono font-bold text-white tracking-tighter">{weight}</span>
                    <button onClick={() => adjustWeight(2.5)} className="w-10 h-10 bg-zinc-900 border border-zinc-700 rounded hover:bg-green-900/30 hover:border-green-700 text-zinc-400 active:scale-95 transition-all">
                        <Plus className="w-5 h-5 mx-auto" />
                    </button>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => adjustWeight(-10)} className="text-[10px] text-zinc-600 font-mono px-2 border border-zinc-800 rounded hover:text-white">-10</button>
                    <button onClick={() => adjustWeight(10)} className="text-[10px] text-zinc-600 font-mono px-2 border border-zinc-800 rounded hover:text-white">+10</button>
                </div>
            </div>
            ...
            <div className="bg-[#050505] p-2 rounded border border-zinc-800 flex flex-col items-center gap-2">
                ...
                <div className="flex items-center w-full justify-between">
                    <button onClick={() => adjustReps(-1)} className="w-10 h-10 bg-zinc-900 border border-zinc-700 rounded hover:bg-red-900/30 hover:border-red-700 text-zinc-400 active:scale-95 transition-all">
                        <Minus className="w-5 h-5 mx-auto" />
                    </button>
                    <span className="text-3xl font-mono font-bold text-white tracking-tighter">{reps}</span>
                    <button onClick={() => adjustReps(1)} className="w-10 h-10 bg-zinc-900 border border-zinc-700 rounded hover:bg-green-900/30 hover:border-green-700 text-zinc-400 active:scale-95 transition-all">
                        <Plus className="w-5 h-5 mx-auto" />
                    </button>
                </div>
        ...
        <div>
            <div className="flex justify-between items-end mb-2">
                <span className="text-[10px] font-bold uppercase text-zinc-500 tracking-widest">Rate Perceived Exertion</span>
                {rpe && <span className="text-xs font-mono font-bold text-magma">RPE {rpe} Selected</span>}
            </div>
        ...
        <button
            ...
            `}
            style={{ clipPath: 'polygon(5% 0, 100% 0, 100% 80%, 95% 100%, 0 100%, 0 20%)' }} // Industrial cut corners
        >
