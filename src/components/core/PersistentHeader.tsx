...
export const PersistentHeader: React.FC<PersistentHeaderProps> = ({ level, xp, gold, faction, className = '' }) => {
    return (
        <div className={`fixed top-0 left-0 right-0 z-40 p-4 pointer-events-none flex justify-center md:justify-start ${className}`}>
            <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-full px-6 py-2 flex items-center gap-6 shadow-xl pointer-events-auto">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="hidden md:flex flex-col">
                        <span className="text-[10px] text-zinc-400 uppercase tracking-widest leading-none">Level</span>
                ...
                {/* Faction */}
                <div className={`text-xs font-black italic tracking-wider ${faction === 'ALLIANCE' ? 'text-blue-500' : 'text-red-500'}`}>
                    {faction}
                </div>
            </div>
        </div>
