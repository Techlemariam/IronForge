...
const VisionRepCounter: React.FC<VisionRepCounterProps> = ({ isActive, onRepCount, onClose }) => {
    const videoRef = useRef<HTMLVideoElement>(null);
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const streamRef = useRef<MediaStream | null>(null);
    const requestRef = useRef<number | null>(null);
    const [metrics, setMetrics] = useState({ velocity: '0.00', isBelowParallel: false, state: 'IDLE' });
            ...
            <div className="absolute inset-0 pointer-events-none z-20">
                ...
                <div className="absolute top-4 right-12 flex flex-col items-end">
                    <div className="flex items-center gap-2">
                        <span className="text-2xl font-black text-white font-mono">{metrics.velocity}</span>
                        <span className="text-[10px] text-zinc-400 uppercase">m/s</span>
                    </div>
                    {parseFloat(metrics.velocity) > 0 && parseFloat(metrics.velocity) < 0.3 && (
                        <span className="text-red-500 font-bold uppercase text-[10px] animate-pulse">Drive! Too Slow!</span>
                    )}
                </div>

                {/* Depth Indicator */}
                <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded border ${metrics.isBelowParallel ? 'bg-green-500/20 border-green-500 text-green-400' : 'bg-black/50 border-zinc-700 text-zinc-500'}`}>
                    <span className="text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                        <ArrowDown className={`w-4 h-4 ${metrics.isBelowParallel ? 'animate-bounce' : ''}`} />
