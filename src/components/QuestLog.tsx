...
import { Scroll, Sword, Map, Lock, Star, Sparkles, Hammer, CheckCircle2, ChevronRight } from 'lucide-react';
...
const QuestLog: React.FC<QuestLogProps> = ({ sessions, history, onSelectSession, level }) => {
  ...
  const acts = [
    { id: 'ACT_I', label: 'Act I: Initiation', icon: <Map className="w-4 h-4" />, desc: 'Standard Protocol' },
    { id: 'ACT_II', label: 'Act II: The Oracle', icon: <Sparkles className="w-4 h-4" />, desc: 'AI Generated' },
    { id: 'ACT_III', label: 'Act III: Architect', icon: <Hammer className="w-4 h-4" />, desc: 'Custom Dungeons' },
  ];
          ...
          <button
            ...
            className={`flex-1 py-4 px-2 flex flex-col items-center gap-1 transition-all relative overflow-hidden group
              ${activeAct === act.id 
                ...
                : 'bg-[#0a0a0a] text-zinc-600 hover:text-zinc-400 hover:bg-[#111]'}
            `}
          >
            {activeAct === act.id && (
                <div className="absolute top-0 left-0 w-full h-0.5 bg-[#c79c6e] shadow-[0_0_10px_#c79c6e]" />
            )}
            <div className={`p-1 rounded ${activeAct === act.id ? 'bg-[#c79c6e]/10' : ''}`}>
                {act.icon}
            </div>
        ...
        <div className="mb-6 flex items-center justify-between">
            <div>
                ...
                <p className="text-xs text-zinc-500 font-sans">
                    ...
                    {activeAct === 'ACT_II' && "Dynamic challenges forged by the Spirit Guide based on your biometrics."}
        ...
        <div className="space-y-4">
            {filteredSessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-zinc-800 rounded-lg opacity-50">
                    ...
                    <p className="text-zinc-500 font-serif text-sm">No Quests Available in this Region.</p>
                    {activeAct === 'ACT_II' && <p className="text-[10px] text-zinc-600 mt-1">Consult the Oracle to generate new paths.</p>}
                    {activeAct === 'ACT_III' && <p className="text-[10px] text-zinc-600 mt-1">Use the Dungeon Builder to construct this area.</p>}
                </div>
            ) : (
                filteredSessions.map((session, idx) => {
                    ...
                    if (isGenerated) {
                        borderColor = 'border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.15)] group-hover:border-purple-400';
                        iconColor = 'bg-purple-900 border-purple-500 text-purple-200';
                        typeLabel = 'Prophecy';
                    } else if (isCustom) {
                        borderColor = 'border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.15)] group-hover:border-green-400';
                    ...
                    return (
                        ...
                        >
                            {/* Icon Box */}
                            <div className={`mt-1 p-3 rounded border shadow-inner transition-colors ${iconColor}`}>
                                {isGenerated ? <Sparkles className="w-6 h-6 animate-pulse" /> : isCustom ? <Hammer className="w-6 h-6"/> : <Sword className="w-6 h-6" />}
                            </div>
