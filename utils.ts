...
import { Session, Block, Exercise, Set as WorkoutSet, ExerciseLogic, Rarity, SkillNode, IntervalsWellness, TitanAttributes, MeditationLog } from './types';
import { ACHIEVEMENTS, TITAN_RANKS, SKILL_TREE, SESSIONS } from './data';
...
export const checkGate = (rankId: number, unlockedIds: Set<string>): boolean => {
    ...
    if (rankId === 2) {
        ...
        const hasProfession = ACHIEVEMENTS.some(a => a.category === 'professions' && unlockedIds.has(a.id));
    ...
    if (rankId === 3) {
        const featsCount = ACHIEVEMENTS.filter(a => a.category === 'feats' && unlockedIds.has(a.id)).length;
        return featsCount >= 3;
    }
    ...
    if (rankId === 5) {
        ...
        const hasRaidMastery = unlockedIds.has('defender_ironforge') || unlockedIds.has('clear_deadmines');
...
export const calculateTitanRank = (unlockedIds: Set<string>) => {
  ...
  const talentPoints = ACHIEVEMENTS
    .filter(a => unlockedIds.has(a.id) && a.category !== 'cardio')
    .reduce((acc, a) => acc + a.points, 0);

  const kineticShards = ACHIEVEMENTS
    .filter(a => unlockedIds.has(a.id) && a.category === 'cardio')
  ...
  }) || { id: 0, name: 'Unranked', minTp: 0, minKs: 0, gateDescription: 'Complete quests to earn rank.' };
...
export const calculateTitanAttributes = (
    ...
    const strengthUnlocked = SKILL_TREE.filter(n => (n.category === 'push' || n.category === 'legs') && purchasedSkills.has(n.id)).length;
    ...
    const endUnlocked = SKILL_TREE.filter(n => n.category === 'endurance' && purchasedSkills.has(n.id)).length;
    let vo2Bonus = 0;
    ...
    const pullSkills = SKILL_TREE.filter(n => n.category === 'pull' && purchasedSkills.has(n.id)).length;
    ...
    const techCheevos = ACHIEVEMENTS.filter(a => a.category === 'professions' && unlockedIds.has(a.id)).length;
    const coreSkills = SKILL_TREE.filter(n => n.category === 'core' && purchasedSkills.has(n.id)).length;
...
class AudioController {
    ...
    public play(type: 'ding' | 'quest_accept' | 'fail' | 'loot_epic' | 'achievement') {
...
export const playSound = (type: 'ding' | 'quest_accept' | 'fail' | 'loot_epic' | 'achievement') => {
    ...
    if (block.type === 'station' && block.exercises) {
      block.exercises.forEach((ex: Exercise) => {
        if (ex.logic === ExerciseLogic.TM_PERCENT) {
          ...
          ex.instructions = ['Debuff Active: -40% Intensity.', 'Objective: Survive.', ...(ex.instructions || [])];
          
          const newSets: WorkoutSet[] = [];
          ex.sets.forEach((set: WorkoutSet, idx: number) => {
             if (idx < 3) { 
               const newSet = {...set};
               if (newSet.weightPct && newSet.weightPct > 0.60) {
                 newSet.weightPct = 0.60;
